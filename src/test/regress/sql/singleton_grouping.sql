...
-- Singleton paths are alternatives, not a replacement for ordinary grouping.
-- A larger permanent relation is required because temporary tables cannot be
-- scanned in parallel.
CREATE TABLE singleton_parallel_part (
    bucket int NOT NULL,
    id int NOT NULL,
...
CREATE TABLE singleton_parallel_part_1
    PARTITION OF singleton_parallel_part FOR VALUES FROM (0) TO (1);
CREATE TABLE singleton_parallel_part_2
    PARTITION OF singleton_parallel_part FOR VALUES FROM (1) TO (2);
INSERT INTO singleton_parallel_part
SELECT g % 2, g, (g * 37) % 1000, 'payload' || g
FROM generate_series(1, 10000) g;
ALTER TABLE singleton_parallel_part_1 SET (parallel_workers = 2);
ALTER TABLE singleton_parallel_part_2 SET (parallel_workers = 2);
CREATE FUNCTION singleton_safe_slow(p text) RETURNS text
...
BEGIN
    ...
    RETURN r;
END $$;
CREATE FUNCTION singleton_unsafe_slow(p text) RETURNS text
LANGUAGE plpgsql VOLATILE STRICT PARALLEL UNSAFE COST 100 AS $$
DECLARE r text := p;
BEGIN
    r := md5(p || r);
    RETURN r;
...
-- A direct partial path allows worker-local sorting and Gather Merge.
BEGIN;
SET LOCAL enable_partitionwise_aggregate = off;
...
EXPLAIN (COSTS OFF)
SELECT bucket, id, rank_key FROM singleton_parallel_part
GROUP BY bucket, id, rank_key ORDER BY rank_key + 1;
ROLLBACK;

-- With ordinary grouping methods disabled, the direct partial path remains.
-- A parallel-safe output expression is projected below Gather.
BEGIN;
SET LOCAL enable_partitionwise_aggregate = off;
SET LOCAL enable_parallel_append = on;
SET LOCAL max_parallel_workers_per_gather = 2;
...
EXPLAIN (COSTS OFF)
SELECT bucket, id, singleton_safe_slow(payload)
FROM singleton_parallel_part GROUP BY bucket, id;
EXPLAIN (COSTS OFF)
SELECT bucket, id, singleton_unsafe_slow(payload)
FROM singleton_parallel_part GROUP BY bucket, id;
ROLLBACK;

-- Partitionwise aggregate remains available and can win when its child-wise
-- estimates are favorable.  It must not be erased by the singleton alternative.
BEGIN;
...
GROUP BY bucket, id, rank_key ORDER BY rank_key + 1;
ROLLBACK;
DROP FUNCTION singleton_safe_slow(text);
DROP FUNCTION singleton_unsafe_slow(text);
DROP TABLE singleton_parallel_part CASCADE;
...
-- A UNION parent needs sorted children, so they stay rejected.
EXPLAIN (COSTS OFF)
SELECT id FROM singleton_base GROUP BY id
UNION
SELECT id FROM singleton_base GROUP BY id;

-- A UNION ALL parent does not require ordered children, so each leaf may use
-- its own singleton proof.
EXPLAIN (COSTS OFF)
SELECT id FROM singleton_base GROUP BY id
UNION ALL
SELECT id FROM singleton_base GROUP BY id;
...
-- With partial retrieval, keep ordinary grouping when the target contains a
-- volatile function.  LIMIT should observe only three evaluations.
SELECT setval('singleton_expr_calls', 1, false);
EXPLAIN (COSTS OFF)
SELECT id, nextval('singleton_expr_calls') AS calls
FROM singleton_base
GROUP BY id
ORDER BY id
LIMIT 3;
...
ORDER BY id
LIMIT 3;
SELECT last_value, is_called, last_value = 3 AS calls_ok
FROM singleton_expr_calls;
