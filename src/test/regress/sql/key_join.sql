...
CREATE VIEW vcusage_parent_view AS
SELECT id
FROM vcusage_parent;

CREATE VIEW vcusage_key_via_plain AS
SELECT c.id AS child_id, p.id AS parent_id
FROM vcusage_parent_view p
JOIN vcusage_child c FOR KEY (parent_id) -> p (id);

SELECT d.deptype,
       d.refclassid::regclass::text AS refclass,
       count(*) AS ndeps
FROM pg_depend d
JOIN pg_rewrite r ON r.oid = d.objid
JOIN pg_class v ON v.oid = r.ev_class
WHERE d.classid = 'pg_rewrite'::regclass
  AND v.relname = 'vcusage_key_via_plain'
  AND d.refclassid IN ('pg_class'::regclass, 'pg_constraint'::regclass)
  AND d.deptype IN ('n', 'k')
GROUP BY d.deptype, d.refclassid
ORDER BY d.deptype, refclass;

SELECT count(*) AS normal_constraint_deps
FROM pg_depend d
JOIN pg_rewrite r ON r.oid = d.objid
JOIN pg_class v ON v.oid = r.ev_class
WHERE d.classid = 'pg_rewrite'::regclass
...
CREATE OR REPLACE VIEW vcusage_parent_view AS
SELECT id
FROM vcusage_parent
WHERE id > 0;

ALTER TABLE vcusage_parent ENABLE ROW LEVEL SECURITY;

DROP VIEW vcusage_key_via_plain, vcusage_parent_view, vcusage_key_view,
          vcusage_plain_view;
