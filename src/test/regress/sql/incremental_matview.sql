...
CREATE MATERIALIZED VIEW mv_ivm_1 WITH (incremental_view_maintenance) AS SELECT i,j,k FROM mv_base_a a INNER JOIN mv_base_b b USING(i) WITH NO DATA;
...
CREATE  MATERIALIZED VIEW mv_ivm_distinct WITH (incremental_view_maintenance) AS SELECT DISTINCT * FROM mv_base_a;
...
CREATE MATERIALIZED VIEW mv_ivm_func WITH (incremental_view_maintenance) AS SELECT * FROM ivm_func();
CREATE MATERIALIZED VIEW mv_ivm_no_tbl WITH (incremental_view_maintenance) AS SELECT 1;
ROLLBACK;

-- result of materialized view have duplicate result.
BEGIN;
CREATE MATERIALIZED VIEW mv_ivm_duplicate WITH (incremental_view_maintenance) AS SELECT j FROM mv_base_a;
...
CREATE MATERIALIZED VIEW mv_ivm_agg WITH (incremental_view_maintenance) AS SELECT i, SUM(j), COUNT(i), AVG(j) FROM mv_base_a GROUP BY i;
...
CREATE MATERIALIZED VIEW mv_self(v1, v2) WITH (incremental_view_maintenance) AS
...
CREATE MATERIALIZED VIEW mv(v1, v2) WITH (incremental_view_maintenance) AS
...
CREATE MATERIALIZED VIEW mv_ri(i1, i2) WITH (incremental_view_maintenance) AS
...
CREATE MATERIALIZED VIEW mv WITH (incremental_view_maintenance) AS SELECT * FROM base_t;
...
CREATE MATERIALIZED VIEW mv_mytype WITH (incremental_view_maintenance) AS
...
CREATE MATERIALIZED VIEW mv(a,b) WITH (incremental_view_maintenance) AS SELECT a.i, b.i FROM mv_base_a a LEFT JOIN mv_base_b b ON a.i=b.i;
-- CTE is not supported
CREATE MATERIALIZED VIEW mv WITH (incremental_view_maintenance) AS
    WITH b AS ( SELECT * FROM mv_base_b) SELECT a.i,a.j FROM mv_base_a a, b WHERE a.i = b.i;
-- contain system column
CREATE MATERIALIZED VIEW mv_ivm01 WITH (incremental_view_maintenance) AS SELECT i,j,xmin FROM mv_base_a;
CREATE MATERIALIZED VIEW mv_ivm02 WITH (incremental_view_maintenance) AS SELECT i,j FROM mv_base_a WHERE xmin = '610';
CREATE MATERIALIZED VIEW mv_ivm04 WITH (incremental_view_maintenance) AS SELECT i,j,xmin::text AS x_min FROM mv_base_a;
CREATE MATERIALIZED VIEW mv_ivm06 WITH (incremental_view_maintenance) AS SELECT i,j,xidsend(xmin) AS x_min FROM mv_base_a;
-- contain subquery
CREATE MATERIALIZED VIEW mv_ivm03 WITH (incremental_view_maintenance) AS SELECT i,j FROM mv_base_a WHERE i IN (SELECT i FROM mv_base_b WHERE k < 103 );
CREATE MATERIALIZED VIEW mv_ivm04 WITH (incremental_view_maintenance) AS SELECT a.i,a.j FROM mv_base_a a, (SELECT * FROM mv_base_b) b WHERE a.i = b.i;
CREATE MATERIALIZED VIEW mv_ivm05 WITH (incremental_view_maintenance) AS SELECT i,j, (SELECT k FROM mv_base_b b WHERE a.i = b.i) FROM mv_base_a a;
-- contain ORDER BY
CREATE MATERIALIZED VIEW mv_ivm07 WITH (incremental_view_maintenance) AS SELECT i,j,k FROM mv_base_a a INNER JOIN mv_base_b b USING(i) ORDER BY i,j,k;
-- contain HAVING
CREATE MATERIALIZED VIEW mv_ivm08 WITH (incremental_view_maintenance) AS SELECT i,j,k FROM mv_base_a a INNER JOIN mv_base_b b USING(i) GROUP BY i,j,k HAVING SUM(i) > 5;
...
CREATE MATERIALIZED VIEW  mv_ivm07 WITH (incremental_view_maintenance) AS SELECT a.i,a.j FROM mv_base_a a,b_view b WHERE a.i = b.i;
CREATE MATERIALIZED VIEW  mv_ivm08 WITH (incremental_view_maintenance) AS SELECT a.i,a.j FROM mv_base_a a,b_mview b WHERE a.i = b.i;
CREATE MATERIALIZED VIEW  mv_ivm09 WITH (incremental_view_maintenance) AS SELECT a.i,a.j FROM mv_base_a a, (SELECT i, COUNT(*) FROM mv_base_b GROUP BY i) b WHERE a.i = b.i;
CREATE MATERIALIZED VIEW  mv_ivm10 WITH (incremental_view_maintenance) AS SELECT a.i,a.j FROM mv_base_a a WHERE EXISTS(SELECT 1 FROM mv_base_b b WHERE a.i = b.i) OR a.i > 5;

-- contain mutable functions
CREATE MATERIALIZED VIEW  mv_ivm12 WITH (incremental_view_maintenance) AS SELECT i,j FROM mv_base_a WHERE i = random()::int;

-- LIMIT/OFFSET is not supported
CREATE MATERIALIZED VIEW  mv_ivm13 WITH (incremental_view_maintenance) AS SELECT i,j FROM mv_base_a LIMIT 10 OFFSET 5;

-- DISTINCT ON is not supported
CREATE MATERIALIZED VIEW  mv_ivm14 WITH (incremental_view_maintenance) AS SELECT DISTINCT ON(i) i, j FROM mv_base_a;

-- TABLESAMPLE clause is not supported
CREATE MATERIALIZED VIEW  mv_ivm15 WITH (incremental_view_maintenance) AS SELECT i, j FROM mv_base_a TABLESAMPLE SYSTEM(50);

-- window functions are not supported
CREATE MATERIALIZED VIEW  mv_ivm16 WITH (incremental_view_maintenance) AS SELECT *, cume_dist() OVER (ORDER BY i) AS rank FROM mv_base_a;

-- inheritance parent is not supported
BEGIN;
CREATE TABLE parent (i int, v int);
CREATE TABLE child_a(options text) INHERITS(parent);
CREATE MATERIALIZED VIEW  mv_ivm21 WITH (incremental_view_maintenance) AS SELECT * FROM parent;
ROLLBACK;

-- UNION statement is not supported
CREATE MATERIALIZED VIEW  mv_ivm22 WITH (incremental_view_maintenance) AS SELECT i,j FROM mv_base_a UNION ALL SELECT i,k FROM mv_base_b;;

-- empty target list is not allowed with IVM
CREATE MATERIALIZED VIEW  mv_ivm25 WITH (incremental_view_maintenance) AS SELECT FROM mv_base_a;

-- FOR UPDATE/SHARE is not supported
CREATE MATERIALIZED VIEW  mv_ivm26 WITH (incremental_view_maintenance) AS SELECT i,j FROM mv_base_a FOR UPDATE;

-- tartget list cannot contain ivm column that start with '__ivm'
CREATE MATERIALIZED VIEW  mv_ivm28 WITH (incremental_view_maintenance) AS SELECT i AS "__ivm_count__" FROM mv_base_a;

-- expressions specified in GROUP BY must appear in the target list.
CREATE MATERIALIZED VIEW  mv_ivm29 WITH (incremental_view_maintenance) AS SELECT COUNT(i) FROM mv_base_a GROUP BY i;

-- VALUES is not supported
CREATE MATERIALIZED VIEW mv_ivm_only_values1 WITH (incremental_view_maintenance) AS values(1);
...
CREATE MATERIALIZED VIEW  ivm_rls WITH (incremental_view_maintenance) AS SELECT * FROM rls_tbl;
...
---
SET SESSION AUTHORIZATION regress_ivm_user;
CREATE MATERIALIZED VIEW ivm_rls2 WITH (incremental_view_maintenance) AS SELECT * FROM rls_tbl JOIN num_tbl USING(id);
RESET SESSION AUTHORIZATION;
...
CREATE MATERIALIZED VIEW mv_update_same WITH (incremental_view_maintenance) AS SELECT * FROM tbl_update_same;
...
CREATE MATERIALIZED VIEW mv_self_ref WITH (incremental_view_maintenance) AS SELECT * FROM tbl_self_ref;
...
CREATE MATERIALIZED VIEW mv_idx3(i_a, i_b) WITH (incremental_view_maintenance) AS SELECT a.i, b.i FROM base_a a, base_b b;

--- missing some pkey columns: no index
CREATE MATERIALIZED VIEW mv_idx4 WITH (incremental_view_maintenance) AS SELECT j FROM base_a;
CREATE MATERIALIZED VIEW mv_idx5 WITH (incremental_view_maintenance) AS SELECT a.i, b.j FROM base_a a, base_b b;

--- with set-returning function: no index
CREATE MATERIALIZED VIEW mv_idx6 WITH (incremental_view_maintenance) AS SELECT i FROM base_a, generate_series(1,10);
...
CREATE MATERIALIZED VIEW mv_json WITH (incremental_view_maintenance) AS SELECT * from table_json;
ROLLBACK;
