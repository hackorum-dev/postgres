...
-- Deduplicate the to-many side before the join, so the join does not produce
-- the duplicate rows that DISTINCT would only discard.
EXPLAIN (VERBOSE, COSTS OFF)
...
-- Nor for DISTINCT ON, which keeps a particular row from each group, even
-- though nothing of the to-many side is projected here
EXPLAIN (VERBOSE, COSTS OFF)
...
-- count(DISTINCT) discards the duplicate rows the join produces, so they need
...
-- An aggregate that counts the duplicates needs every one of them
EXPLAIN (COSTS OFF)
SELECT count(*)
  FROM eager_distinct_a1 a1
  JOIN eager_distinct_a2 a2 ON a2.a1_id = a1.id
...
-- A HAVING clause naming the same aggregate is no obstacle
EXPLAIN (COSTS OFF)
SELECT count(DISTINCT a1.id)
  FROM eager_distinct_a1 a1
  JOIN eager_distinct_a2 a2 ON a2.a1_id = a1.id
...
-- ... but one that counts the duplicates is
EXPLAIN (COSTS OFF)
SELECT count(DISTINCT a1.id)
  FROM eager_distinct_a1 a1
  JOIN eager_distinct_a2 a2 ON a2.a1_id = a1.id
...
-- Here the side worth deduplicating is the nullable one, which nothing may
-- be pushed to, so the count is taken over the join as it stands
EXPLAIN (COSTS OFF)
SELECT count(DISTINCT a1.id)
  FROM eager_distinct_a1 a1
...
-- it keeps that row observable, so the relation producing it is not reduced to
-- an existence check
--
...
-- Counting the matches still requires every one of them
EXPLAIN (COSTS OFF)
SELECT d.id, count(*)
  FROM eager_minmax_d d
  JOIN eager_minmax_f1 f1 ON f1.d_id = d.id
...
-- A NULL join key matches nothing, whether the join produces the matches or
-- only tests for them
EXPLAIN (COSTS OFF)
SELECT DISTINCT d.id
  FROM eager_edge_d d
...
-- A lateral reference reads a column of f that no grouping key covers, so the
...
-- An existence check the query wrote itself is no obstacle
EXPLAIN (COSTS OFF)
SELECT DISTINCT d.id
  FROM eager_edge_d d
  JOIN eager_edge_f f ON f.d_id = d.id
