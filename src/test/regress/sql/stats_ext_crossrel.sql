...
CREATE STATISTICS bad_stats1 (mcv) ON t1.val;
CREATE STATISTICS bad_stats2 (mcv) ON t1.val FROM t2, t1;
CREATE STATISTICS bad_stats3 (mcv) FROM t2 JOIN t1 ON (t2.t1_id = t1.id);
CREATE STATISTICS bad_stats4 (mcv) ON val FROM t2 JOIN t1 ON (t2.t1_id = t1.id);
CREATE STATISTICS bad_stats5 (mcv) ON lower(t1.val) FROM t2 JOIN t1 ON (t2.t1_id = t1.id);
-- Composite join condition (AND) is not supported
CREATE STATISTICS bad_composite (mcv) ON t1.val
FROM t2 JOIN t1 ON (t2.t1_id = t1.id AND t2.id = t1.id);
-- Non-Var operand in join condition (constant)
CREATE STATISTICS bad_const_join (mcv) ON t1.val
FROM t2 JOIN t1 ON (t1.id = 1);
-- Non-equality join condition
CREATE STATISTICS bad_non_eq (mcv) ON t1.val
FROM t2 JOIN t1 ON (t2.t1_id > t1.id);
-- Non-inner join types are not supported
CREATE STATISTICS bad_left (mcv) ON t1.val
FROM t2 LEFT JOIN t1 ON (t2.t1_id = t1.id);
CREATE STATISTICS bad_right (mcv) ON t1.val
FROM t2 RIGHT JOIN t1 ON (t2.t1_id = t1.id);
CREATE STATISTICS bad_full (mcv) ON t1.val
FROM t2 FULL JOIN t1 ON (t2.t1_id = t1.id);
CREATE STATISTICS bad_cross (mcv) ON t1.val
FROM t2 CROSS JOIN t1;
-- Whole-row reference in join condition is not supported
CREATE STATISTICS bad_wholerow (mcv) ON t1.val
FROM t2 JOIN t1 ON (t2 = t1);
-- Subquery in FROM clause is not supported
CREATE STATISTICS bad_stats6 (mcv) ON x.val FROM (SELECT * FROM t1) x JOIN t2 ON (x.id = t2.t1_id);
...
FROM t2 JOIN no_idx ON (t2.t1_id = no_idx.id);
...
CREATE TABLE t3 (id INTEGER PRIMARY KEY, label TEXT NOT NULL);
...
CREATE STATISTICS nway_multi_filter (mcv) ON t1.val, t3.label
    FROM t2
    JOIN t1 ON (t2.t1_id = t1.id)
    JOIN t3 ON (t1.id = t3.id);

-- keyrefs should be {2, 3}: val from varno 2=t1,
-- label from varno 3=t3
SELECT s.stxname,
       s.stxrelid::regclass,
...
CREATE STATISTICS nway_stats (mcv) ON t1.val
    FROM t2
    JOIN t1 ON (t2.t1_id = t1.id)
    JOIN t3 ON (t1.id = t3.id);
...
-- jstats_one_col tracks keywords.keyword (attnum 2 in keywords, stxkeyrefs={2}).
...
SELECT s.stxname, s.stxkeys, s.stxkeyrefs
FROM pg_statistic_ext s
