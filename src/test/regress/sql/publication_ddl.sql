...
-- create publication for one table with two columns and a condition with an expression
...
CREATE PUBLICATION testpub_ddl_10 FOR TABLE testpub_ddl_tbl5("bar beque");
...
CREATE PUBLICATION "Quoted Pub" FOR TABLE "Pub Schema"."Quoted Table" ("Col One", "select") WHERE ("Col One" > 0);
SELECT pg_get_publication_ddl('Quoted Pub');
SELECT pg_get_publication_ddl((SELECT oid FROM pg_publication WHERE pubname='Quoted Pub'));
SELECT pg_get_publication_ddl('Quoted Pub', 'pretty', 'true');
