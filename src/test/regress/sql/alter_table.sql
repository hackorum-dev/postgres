...
-- The leaf can use a different tablespace from its parent index.
alter index at_reb_expr_leaf set tablespace pg_default;

-- Load the leaf indexes with properties not preserved by the cloned definition.
comment on index at_reb_expr_leaf is 'leaf expr index comment';
comment on index at_reb_expr_leaf_2 is 'second leaf expr index comment';
...
-- The leaf's explicitly different tablespace must survive the rebuild.
select reltablespace = 0 as uses_default_tablespace
  from pg_class
  where oid = 'at_reb_expr_leaf'::regclass;
