...
create table at_reb_set_plain (val int not null,
  gen int generated always as (val + 1) stored);
create index at_reb_set_plain_idx on at_reb_set_plain ((gen + 1));
alter index at_reb_set_plain_idx alter column 1 set statistics 654;
alter table at_reb_set_plain alter column gen set expression as (val + 2);
select attstattarget
  from pg_attribute
  where attrelid = 'at_reb_set_plain_idx'::regclass and attnum = 1;
drop table at_reb_set_plain;
