...
-- Case: DDL executed by a DEFAULT expression evaluated during rewrite,
-- with no event trigger involved.
CREATE TABLE rewrite_default_target (a int);
INSERT INTO rewrite_default_target VALUES (1);

SET client_min_messages = 'ERROR';
CREATE PUBLICATION rewrite_pub2;
RESET client_min_messages;

CREATE FUNCTION test_default_add_table() RETURNS int
LANGUAGE plpgsql VOLATILE AS $$
BEGIN
  EXECUTE 'ALTER PUBLICATION rewrite_pub2 ADD TABLE rewrite_default_target';
  RETURN 1;
END;
$$;

-- Should fail
ALTER TABLE rewrite_default_target
  SET UNLOGGED,
  ADD COLUMN b int DEFAULT test_default_add_table();

-- Cleanup
DROP FUNCTION test_default_add_table();
DROP PUBLICATION rewrite_pub2;
DROP TABLE rewrite_default_target;
