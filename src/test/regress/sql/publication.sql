...
CREATE PUBLICATION testpub_schema_except_setsch
    FOR TABLES IN SCHEMA pub_test EXCEPT (TABLE pub_test.testpub_tbl_s2);
\dRp+ testpub_schema_except_setsch
ALTER TABLE pub_test.testpub_tbl_s2 SET SCHEMA public;
\dRp+ testpub_schema_except_setsch
...
-- Check that a table excluded from a schema publication is reported only as
-- excluded and not as published.
\d pub_test.testpub_nopk

-- fail: ADD TABLE for a partition is rejected when a partition ancestor
-- is currently in the publication's EXCEPT list.
CREATE PUBLICATION testpub_except_ancestor
    FOR TABLES IN SCHEMA pub_test EXCEPT (TABLE pub_test.testpub_parted_s);
ALTER PUBLICATION testpub_except_ancestor ADD TABLE pub_test.testpub_part_s;
DROP PUBLICATION testpub_except_ancestor;

-- fail: same contradiction (explicit partition + EXCEPT-ed ancestor), but
-- given in a single statement instead of two separate ones
CREATE PUBLICATION testpub_except_ancestor_same_create
    FOR TABLE pub_test.testpub_part_s,
       TABLES IN SCHEMA pub_test EXCEPT (TABLE pub_test.testpub_parted_s);

-- fail: same contradiction as above, but the partition lives in a different
-- schema (public) than its EXCEPT-ed root (pub_test)
CREATE TABLE public.testpub_part_s2 PARTITION OF pub_test.testpub_parted_s FOR VALUES IN (2);
CREATE PUBLICATION testpub_except_ancestor_cross_schema
    FOR TABLES IN SCHEMA pub_test EXCEPT (TABLE pub_test.testpub_parted_s),
       TABLE public.testpub_part_s2;
DROP TABLE public.testpub_part_s2;

-- ALTER PUBLICATION ... SET replaces the publication's whole relation list, so
-- an existing entry for a relation now requested with the opposite kind must be
...
-- and the reverse direction: an ordinary member becomes an EXCEPT entry
CREATE PUBLICATION testpub_except_alter2 FOR TABLE pub_test.testpub_tbl_s1;
ALTER PUBLICATION testpub_except_alter2
    SET TABLES IN SCHEMA pub_test EXCEPT (TABLE pub_test.testpub_tbl_s1);
\dRp+ testpub_except_alter2
DROP PUBLICATION testpub_except_alter2;
...
CREATE TABLE pub_test.testpub_inh_sibling (b int) INHERITS (pub_test.testpub_inh_parent);
...
-- fail: the child is also explicitly included by the same statement
CREATE PUBLICATION testpub_inh
    FOR TABLES IN SCHEMA pub_test EXCEPT (TABLE pub_test.testpub_inh_parent),
...
CREATE PUBLICATION testpub_inh
    FOR TABLES IN SCHEMA pub_test EXCEPT (TABLE ONLY pub_test.testpub_inh_parent),
        TABLE testpub_inh_child;
\dRp+ testpub_inh
DROP PUBLICATION testpub_inh;

-- An EXCEPT clause in the child's schema also excludes the child explicitly,
-- so both clauses agree and there is no conflict.
CREATE PUBLICATION testpub_inh
    FOR TABLES IN SCHEMA pub_test EXCEPT (TABLE pub_test.testpub_inh_parent),
...
-- SET runs the same conflict checks.  Unlike ADD it replaces the EXCEPT list,
-- so an exclusion the publication already holds is not carried over and cannot
-- conflict with anything; only what the statement itself names can.
CREATE PUBLICATION testpub_xs
    FOR TABLES IN SCHEMA pub_test EXCEPT (TABLE pub_test.testpub_xs_parted);
-- fail: public holds a partition of the root this statement excludes
ALTER PUBLICATION testpub_xs
    SET TABLES IN SCHEMA public, pub_test
        EXCEPT (TABLE pub_test.testpub_xs_parted);
DROP PUBLICATION testpub_xs;
...
-- partition lives in a different schema. Excluding the root also excludes its
-- partition, even though the partition is in another schema. Publishing that
-- schema as well creates a conflict because the partition would be both
-- published and excluded; without publishing the partition's schema, the
-- exclusion still cascades to the partition.
CREATE SCHEMA gpt_cross_sch1;
CREATE SCHEMA gpt_cross_sch2;
CREATE TABLE gpt_cross_sch1.croot (id int) PARTITION BY RANGE (id);
CREATE TABLE gpt_cross_sch2.cpart1 PARTITION OF gpt_cross_sch1.croot FOR VALUES FROM (1) TO (10);

-- fail: cpart1 would be published by its own schema and excluded through its root
CREATE PUBLICATION pub_cross_schema_except FOR TABLES IN SCHEMA gpt_cross_sch1 EXCEPT (TABLE gpt_cross_sch1.croot), TABLES IN SCHEMA gpt_cross_sch2;

SET client_min_messages = 'ERROR';
CREATE PUBLICATION pub_cross_schema_except FOR TABLES IN SCHEMA gpt_cross_sch1 EXCEPT (TABLE gpt_cross_sch1.croot);
RESET client_min_messages;

SELECT * FROM test_gpt(ARRAY['pub_cross_schema_except'], 'gpt_cross_sch1.croot'); -- no result (excluded)
-- no result: the root's exclusion cascades to the partition even though the
-- partition lives in a schema this publication does not mention
SELECT * FROM test_gpt(ARRAY['pub_cross_schema_except'], 'gpt_cross_sch2.cpart1');

DROP PUBLICATION pub_cross_schema_except;
DROP TABLE gpt_cross_sch2.cpart1, gpt_cross_sch1.croot;
DROP SCHEMA gpt_cross_sch1, gpt_cross_sch2;
