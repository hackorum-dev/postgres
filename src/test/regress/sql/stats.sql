...
-- shared stats mid-transaction and must not be double-counted by a later flush.
--
...
-- Flush mid-transaction.  The scan reaches shared stats; the insert is deferred.
SELECT pg_stat_force_next_flush();
SELECT seq_scan - :seq_scan_before AS seq_scan_delta,
...
-- those rolled back count as dead, and the mid-transaction scan is still there.
SELECT pg_stat_force_next_flush();
...
-- Flush mid-transaction.  The scan reaches shared stats; the rest is deferred.
SELECT pg_stat_force_next_flush();
...
-- Flush mid-transaction.  The scan reaches shared stats; transactional counters
-- are deferred.
SELECT pg_stat_force_next_flush();
