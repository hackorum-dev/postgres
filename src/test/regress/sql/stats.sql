...
--
-- Test that a mid-transaction flush keeps the database aggregate in step
-- with the relation counters that feed into it, rather than a flush behind.
--
CREATE TABLE partial_flush_db(id int) WITH (autovacuum_enabled = off);
INSERT INTO partial_flush_db SELECT generate_series(1, 100);
SELECT pg_stat_force_next_flush();

SELECT pg_stat_get_db_tuples_returned(oid) AS db_tup_returned_before
  FROM pg_database WHERE datname = current_database() \gset

BEGIN;
SET LOCAL stats_fetch_consistency = none;

SELECT count(*) FROM partial_flush_db;

SELECT pg_stat_force_next_flush();

-- The aggregate must have advanced by at least the relation's own scan in
-- this same flush.  It also counts catalog scans, so this is not an equality.
SELECT tup_returned - :db_tup_returned_before >= seq_tup_read AS db_kept_up
  FROM pg_stat_database, pg_stat_user_tables
  WHERE datname = current_database() AND relname = 'partial_flush_db';

COMMIT;

DROP TABLE partial_flush_db;
...
-- Function calls are not transactional, so the two calls are already visible
-- in-transaction after the flush.
