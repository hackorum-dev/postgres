...
-- Also verify that hot/newpage updates (which are subsets of n_tup_upd)
-- were not flushed ahead of n_tup_upd, and tuple counts stay non-negative.
...
--
-- Test in-transaction partial flush: rollback discards transactional counters
-- but non-transactional counters already flushed to shared stats persist.
--
CREATE TABLE partial_flush_rollback(id int);
...
-- Flush in-transaction: scans go to shared stats, inserts stay pending.
SELECT pg_stat_force_next_flush();

-- Verify: scans visible, inserts and live_tup deferred.  Also check that
-- partial flush did not expose hot/newpage without their parent update count.
SELECT seq_scan - :seq_scan_before AS seq_scan_delta,
       seq_tup_read - :seq_tup_read_before AS seq_tup_read_delta,
       n_tup_ins - :n_tup_ins_before AS n_tup_ins_delta,
...
-- After rollback: scans persist (already in shared stats), inserts discarded,
-- n_live_tup unchanged, and hot/newpage remain consistent with n_tup_upd.
SELECT pg_stat_force_next_flush();
SELECT seq_scan - :seq_scan_before AS seq_scan_delta,
       seq_tup_read - :seq_tup_read_before AS seq_tup_read_delta,
       n_live_tup - :n_live_tup_before AS n_live_tup_delta,
...
--
-- Test in-transaction partial flush with TRUNCATE: the truncate's reset of
-- live/dead counters is transactional and must not reach shared stats until
-- commit.
--
CREATE TABLE partial_flush_truncate(id int);
...
-- Case 1: DML + TRUNCATE + DML + ROLLBACK.  The truncate's zeroing and all
-- transactional counters must not leak to shared stats.
BEGIN;
SET LOCAL stats_fetch_consistency = none;
...
-- Mid-transaction flush: scan goes to shared stats, everything else deferred.
SELECT pg_stat_force_next_flush();
...
-- After rollback: live_tup unchanged (rollback undid the inserts before they
-- became live), but dead_tup increases because aborted inserts produce dead
-- tuples that vacuum must clean up.
SELECT pg_stat_force_next_flush();
SELECT n_live_tup = :n_live_tup_before AS live_tup_unchanged,
       n_dead_tup - :n_dead_tup_before AS dead_tup_delta
  FROM pg_stat_user_tables WHERE relname = 'partial_flush_truncate';

-- Case 2: DML + TRUNCATE + INSERT + DELETE + COMMIT.
-- Update baseline to account for changes from case 1.
SELECT seq_scan AS seq_scan_before,
       n_live_tup AS n_live_tup_before,
       n_dead_tup AS n_dead_tup_before
  FROM pg_stat_user_tables WHERE relname = 'partial_flush_truncate' \gset
...
-- Mid-transaction flush: scans visible, transactional counters deferred.
SELECT pg_stat_force_next_flush();
...
-- delta_live = inserted - deleted = 10 - 3 = 7
-- delta_dead = updated + deleted = 0 + 3 = 3
SELECT pg_stat_force_next_flush();
...
-- lose or double-count function call statistics.
--
SET track_functions TO 'all';
...
-- Call inside an explicit transaction.
BEGIN;
SELECT flush_func_test();
SELECT flush_func_test();
SELECT flush_func_test();
...
-- Call with an explicit mid-transaction flush between function calls.
BEGIN;
SELECT flush_func_test();
SELECT flush_func_test();
SELECT pg_stat_force_next_flush();
