...
{
	double group_start = INFINITY;
	double group_stop = -INFINITY;
	double total = 0.0;
	for(int i = 0; i < num_tests; i++)
	{
		double t0 = INSTR_TIME_GET_DOUBLE(test_start[i]);
		double tf = INSTR_TIME_GET_DOUBLE(test_stop[i]);

		group_start = Min(t0, group_start);
		group_stop  = Max(tf, group_stop);
		total += tf - t0;
	}
