...
$result =
  $node->safe_psql('postgres', 'SELECT data_page_checksum_version FROM pg_control_checkpoint();');
...
# as off.
$result =
  $node->safe_psql('postgres', 'SELECT data_page_checksum_version FROM pg_control_init();');
