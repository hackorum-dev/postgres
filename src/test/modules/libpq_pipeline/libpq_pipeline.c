...
test_describe_fetch(PGconn *conn)
{
	...
	/*
	 * Describe must still work, using the cached row descriptor.  (This used
	 * to crash the server.)
	 */
	res = PQdescribePrepared(conn, "fetch_one");
	if (PQresultStatus(res) != PGRES_COMMAND_OK)
		pg_fatal("PQdescribePrepared failed: %s", PQerrorMessage(conn));
	if (PQnfields(res) != 1 || PQftype(res, 0) != INT4OID)
		pg_fatal("expected one int4 column, got %d fields", PQnfields(res));
	PQclear(res);
