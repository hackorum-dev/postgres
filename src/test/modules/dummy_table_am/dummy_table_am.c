 ...
 * Dummy table options.
 *
 * "std" embeds the full standard heap layout: every option that heap's own
 * default_reloptions() understands (fillfactor, toast_tuple_target,
 * parallel_workers, vacuum_index_cleanup, vacuum_truncate,
 * vacuum_max_eager_freeze_failure_rate, autovacuum_enabled,
 * user_catalog_table) is inherited by calling add_reloption_to_kind() on
 * the matching name, and each is registered here so that build_reloptions()
 * fills it with the option's own catalog default when the user does not
 * set it explicitly -- an embedded StdRdOptions field that is never
 * registered stays zeroed, which is not the right "unset" value for
 * several of these (e.g. parallel_workers and
 * vacuum_max_eager_freeze_failure_rate both use -1).  Because "std" is a
 * full StdRdOptions and every one of its fields is populated this way, we
 * can tell core code it's safe to keep reading them directly by setting
 * TableAmRoutine.has_std_options_prefix (see dthandler() below).
 *
...
create_reloptions_table(void)
{
	 ...
	 * core-registered options without duplicating their definitions.  See
	 * the comment on DummyTableOptions.std for why we register all of
	 * them rather than just fillfactor.
	 */
