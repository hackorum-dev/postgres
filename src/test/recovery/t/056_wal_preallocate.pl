...
# A request equal to max_wal_size is reduced if rounding it up to a whole
# segment would exceed the limit.  The force option bypasses that limit.
# Use a max_wal_size that is not a whole number of segments to exercise this.
...
my $limit_segsize = $limit_node->safe_psql('postgres',
	"SELECT pg_size_bytes(current_setting('wal_segment_size'))");
my $limit_bytes = $limit_node->safe_psql('postgres',
	"SELECT pg_size_bytes(current_setting('max_wal_size'))");
($ret, $stdout, $stderr) = $limit_node->psql(
	'postgres', "SELECT pg_wal_preallocate($limit_bytes)");
chomp($stdout);
is($stdout, 4, 'request equal to max_wal_size is limited to four segments');
like($stderr,
	qr/NOTICE:.*request was reduced to fit within "max_wal_size"/,
	'limited request emits a notice');

($ret, $stdout, $stderr) =
  $limit_node->psql('postgres', 'SELECT pg_wal_preallocate()');
chomp($stdout);
is($stdout, 0, 'default call finds the capped pool already warm');
unlike($stderr, qr/NOTICE:/, 'default call is capped without a notice');

is($limit_node->safe_psql(
		'postgres', "SELECT pg_wal_preallocate(5 * $limit_segsize, force => true)"),
	1, 'force bypasses max_wal_size and creates the remaining segment');
$limit_node->stop;
