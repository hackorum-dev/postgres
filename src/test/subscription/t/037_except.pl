...
# the root is excluded via EXCEPT under its own schema's clause, the exclusion
# must cascade to all its partitions, including those in other schemas.
#
# The partition's own schema is deliberately not published here: naming it
# alongside the EXCEPT clause is rejected, because the clause's reach stops at
# its own schema and the two would then disagree about the partition.
$node_publisher->safe_psql(
	'postgres', qq(
	CREATE SCHEMA csch1;
	CREATE SCHEMA csch2;
...
$node_publisher->safe_psql('postgres',
	"CREATE PUBLICATION cross_sch_pub FOR TABLES IN SCHEMA csch1 EXCEPT (TABLE csch1.croot) WITH (publish_via_partition_root = false)"
);
$node_subscriber->safe_psql('postgres',
