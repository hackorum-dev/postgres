...
static DumpId createDumpId(void);
...
main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "acd:E:f:F:gh:l:Op:rsS:tU:vwWx", long_options, &optindex)) != -1)
	{
		switch (c)
		{
			...
			case 'f':
				/* Not put into pgdumpopts; runPgDump() needs its own -f. */
	...
	/* Create an archive file for global commands. */
	if (archDumpFormat != archNull)
	{
	...
	{
		/*
		 * For plain-text output with --clean, print the Drop
		 * databases/Drop tablespaces/Drop roles sections first, ahead of
		 * any creates (see dropDBsText() for why the order matters).  For
		 * non-plain formats, dumpRoles()/dumpTablespaces()/dumpDatabases()
		 * instead attach a drop statement to each archive entry, and the
		 * archiver decides at restore time whether to use it.
		 *
		 ...
		 */
		if (archDumpFormat == archNull && output_clean)
		{
			if (!globals_only && !roles_only && !tablespaces_only)
				dropDBsText(conn);

			if (!roles_only && !no_tablespaces)
				dropTablespacesText(conn);
...
/*
 * Print the "Drop databases"/"Drop tablespaces"/"Drop roles" plain-text
 * sections, in that order, ahead of any "create" sections.  The order
 * matters: a database may depend on a tablespace or role, so those must
 * go first.  Non-plain formats need no such helper; the archiver's
 * restore-time drop pass already runs all drops before any creates,
 * regardless of registration order.
 */
static void
dropDBsText(PGconn *conn)
{
	PGresult   *res;
	int			i;

	Assert(archDumpFormat == archNull);
...
/*
 * Drop tablespaces (see dropDBsText() for why this is plain-text only).
 */
static void
dropTablespacesText(PGconn *conn)
{
	PGresult   *res;
	int			i;
...
/*
 * Drop roles (see dropDBsText() for why this is plain-text only).
 */
static void
dropRolesText(PGconn *conn)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		char	   *rolename = PQgetvalue(res, i, 0);
...
dumpRoles(PGconn *conn)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		appendPQExpBuffer(delQry, "DROP ROLE %s;\n", fmtId(rolename));
		...
		else
		{
			...
			DumpId		roleDumpId = createDumpId();
...
dumpTablespaces(PGconn *conn)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		appendPQExpBuffer(delQry, "DROP TABLESPACE %s;\n", fspcname);
...
dumpDatabases(PGconn *conn)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		 ...
		 * target installation and are never dropped above, for fear of
		 * dropping the DB the restore script is initially connected to. If
		...
		if (archDumpFormat != archNull)
		{
			/* Compute this database's own archive file path. */
			...
			/* Let the archiver drop this database first, if --clean. */
			if (!data_only && !statistics_only && !no_schema &&
				strcmp(dbname, "template1") != 0 && strcmp(dbname, "postgres") != 0)
			{
				resetPQExpBuffer(delQry);
	...
	if (archDumpFormat != archNull)
	{
		...
		{
			snprintf(map_file_path, MAXPGPATH, "%s/map.dat", filename);
			(void) fsync_fname(map_file_path, false);
		}
	}
...
runPgDump(const char *dbname, const char *create_opts, char *dbfile)
{
	...
	/*
	 * If this is not a plain format dump, append the file name and dump
	 * format to the pg_dump command to produce an archive-format dump.
	 */
	if (archDumpFormat != archNull)
	{
		printfPQExpBuffer(&cmd, "\"%s\" %s -f ", pg_dump_bin, pgdumpopts->data);
		appendShellString(&cmd, dbfile);
		appendPQExpBuffer(&cmd, " %s", create_opts);
	...
	else
	{
		...
		if (filename)
		{
			appendPQExpBufferStr(&cmd, " -Fa -f ");
			appendShellString(&cmd, filename);
 ...
 *
 * This is a local, file-scope copy of pg_dump.c's createDumpId() rather
 * than a shared one: that function's home, common.c, is compiled only
 * into the pg_dump binary (pg_dump_sources in meson.build), not into
 * pg_dump_common, the static library pg_dumpall and pg_restore link
 * against, so it isn't available here to call.
 */
static DumpId
createDumpId(void)
{
