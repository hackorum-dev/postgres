...
unsigned long rel_file_size = REL_FILE_SIZE_DEF;		/* in bytes, default 1GB */
...
check_block_file_sizes(void)
{
	...
	if (rel_blck_size < REL_BLCK_SIZE_MIN
		...
		fprintf(stderr, _("rel_block_size %u KB\n"), rel_blck_size >> 10);
	}
	...
	if (rel_file_size < (unsigned long) REL_FILE_SIZE_MIN 
		...
		fprintf(stdout, "rel_file_blck set to %u blocks\n", rel_file_blck);
		fprintf(stdout, "rel_file_blck set to %lu bytes\n", rel_file_size);
		return 1;
	} else {
		fprintf(stderr, _("rel_file_size %lu MB or %u blocks\n"),
			rel_file_size >> 20,
			rel_file_blck);
	}
	...
	if (wal_blck_size < WAL_BLCK_SIZE_MIN
		...
		fprintf(stderr, _("wal_block_size %u KB\n"), wal_blck_size >> 10);
	}
	...
	if (wal_file_size < (unsigned long) WAL_FILE_SIZE_MIN
		...
		fprintf(stderr, _("wal_file_size %lu MB or %u blocks\n"),
			wal_file_size >> 20,
			wal_file_blck);
	}
