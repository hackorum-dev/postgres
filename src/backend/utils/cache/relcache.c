 ...
 * expressions, or the partition key.  For a partitioned table, each partition
 * is checked recursively as well.
 *
...
RelationGetParallelDmlSafety(Relation rel)
{
	...
	if (rel->rd_paralleldml != 0)
		return rel->rd_paralleldml;

	rel->rd_paralleldml = max_parallel_dml_hazard(rel);
