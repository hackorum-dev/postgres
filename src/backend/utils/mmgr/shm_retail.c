...
typedef struct ShmRetailContext
{
	...
	void	 *base;				/* raw address of Postmaster-initialized
								   shared memory */
	chunk_list *chunk_list; 	/* array of pointers to chunks. If
								   ShmRetailContext is permanent, NULL */
} ShmRetailContext;
...
ShmRetailContextCreateInternal(MemoryContext parent,
							   ...
							   void *base,
							   bool isLocal);
static void push_chunk_list(ShmRetailContext *shmContext);
...
static dsa_area *shm_retail_area = NULL;

/* Set and Get DSA area of global meta cache */
void
ShmRetailContextSetDSAarea(dsa_area *area)
{
	if (!shm_retail_area)
		shm_retail_area = area;
}

dsa_area *
ShmRetailContextGetDSAarea(void)
{
	return shm_retail_area;
}
...
ShmRetailContextCreateGlobal(MemoryContext parent,
							 const char *name, void *base)
{
	return ShmRetailContextCreateInternal(parent, name, base, false);
}
							   ...
							   bool isLocal)
{
	...
	if (isLocal)
	{
		...
		/* Assuming parent is local context */
		old_context = MemoryContextSwitchTo(parent);

		shmContext = palloc0(sizeof(ShmRetailContext));
		shmContext->chunk_list = (chunk_list *)palloc0(sizeof(chunk_list));
	...
	else
	{
		 ...
		 */
		RegisterMemoryContextMethods(MEMORY_CONTEXT_SHARED_RETAIL,
		...
		{
			child_lock = (LWLock *)
				MAXALIGN(((char *)shmContext) + sizeof(ShmRetailContext));
	...
	MemoryContextSetShmContextMethodsID(&shmContext->header,
...
{
	return sizeof(ShmRetailContext) + MAXIMUM_ALIGNOF + sizeof(LWLock);
}
...
ShmRetailContextMoveChunk(MemoryContext local_context, MemoryContext global_context)
{
	...
	while (list && list->chunks)
	{
		...
		{
			chunk_backp = dsaptr_to_rawptr(list->chunks[idx], localShmRetailContext->base);
			*(void **) chunk_backp = global_context;
		}
 ...
 */
static void
push_chunk_list(ShmRetailContext *shmContext)
{
	MemoryContext old_context;
	chunk_list *new_list;

	/* choose the same context as current list to make lifetime consistent */
	old_context = MemoryContextSwitchTo(GetMemoryChunkContext(shmContext->chunk_list));

	/* insert a new list into head position */
	new_list = (chunk_list *) palloc0(sizeof(chunk_list));
	new_list->next = shmContext->chunk_list;
	shmContext->chunk_list = new_list;
...
ShmRetailContextReset(MemoryContext context)
{
	...
	while (list && list->chunks)
	{
		...
		for (idx = 0; idx < list->tail; idx++)
			dsa_free(shm_retail_area, list->chunks[idx]);
...
ShmRetailContextAlloc(MemoryContext context, Size size)
{
	...
	/* if shared context is specified, list is not used */
	if (!isLocalShmRetailContext(shmContext))
	{
		dsa_pointer dp;

		dp  = dsa_allocate(shm_retail_area,
						   sizeof(void *) + size);

		chunk_backp = dsaptr_to_rawptr(dp, shmContext->base);
	...
	if (isListFull(shmContext->chunk_list))
		push_chunk_list(shmContext);

	/* Add space for the secret context pointer. */
	list = shmContext->chunk_list;
	list->chunks[list->tail] = dsa_allocate(shm_retail_area,
											sizeof(void *) + size);
