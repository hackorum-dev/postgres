					  ...
					  bool resetcounter, TimestampTz starttime)
{
		...
		rel->rd_rel->relkind != RELKIND_PARTITIONED_TABLE)
	{
		...
		deadtuples -= rel->pgstat_info->tab.counts.delta_dead_tuples -
			rel->pgstat_info->tab.flushed.delta_dead_tuples;
 ...
 * contract they are retained when flushing mid-transaction, in which case only
 * the non-transactional counters are flushed and PGSTAT_FLUSH_PARTIAL is
 * returned.
 *
...
pgstat_relation_flush_cb(PgStat_EntryRef *entry_ref, bool nowait, bool xact_boundary)
{
	 ...
	 * opened by the planner but not used.  A mid-transaction entry with
	 * active transaction state stays pending (PGSTAT_FLUSH_PARTIAL) as commit
	 * will merge more counters into it.
	 */
	...
	tabentry->numscans += lstats->tab.counts.numscans - lstats->tab.flushed.numscans;
	...
	tabentry->tuples_returned += lstats->tab.counts.tuples_returned - lstats->tab.flushed.tuples_returned;
	tabentry->tuples_fetched += lstats->tab.counts.tuples_fetched - lstats->tab.flushed.tuples_fetched;
	tabentry->blocks_fetched += lstats->tab.counts.blocks_fetched - lstats->tab.flushed.blocks_fetched;
	tabentry->blocks_hit += lstats->tab.counts.blocks_hit - lstats->tab.flushed.blocks_hit;
	...
	if (flush_txn)
	{
		tabentry->tuples_inserted += lstats->tab.counts.tuples_inserted - lstats->tab.flushed.tuples_inserted;
		tabentry->tuples_updated += lstats->tab.counts.tuples_updated - lstats->tab.flushed.tuples_updated;
		tabentry->tuples_deleted += lstats->tab.counts.tuples_deleted - lstats->tab.flushed.tuples_deleted;
		tabentry->tuples_hot_updated += lstats->tab.counts.tuples_hot_updated - lstats->tab.flushed.tuples_hot_updated;
		tabentry->tuples_newpage_updated += lstats->tab.counts.tuples_newpage_updated - lstats->tab.flushed.tuples_newpage_updated;

		/*
		 * If table was truncated/dropped, first reset the live/dead counters.
		 * Commit zeroed counts.delta_live/dead_tuples, so zero their stale
		 * flushed baselines too.  changed_tuples is not zeroed on truncate,
		 * so its baseline is still valid.
		 */
		if (lstats->tab.counts.truncdropped && !lstats->tab.flushed.truncdropped)
		{
			tabentry->live_tuples = 0;
			tabentry->dead_tuples = 0;
			tabentry->ins_since_vacuum = 0;
			lstats->tab.flushed.delta_live_tuples = 0;
			lstats->tab.flushed.delta_dead_tuples = 0;
		}

		tabentry->live_tuples += lstats->tab.counts.delta_live_tuples - lstats->tab.flushed.delta_live_tuples;
		tabentry->dead_tuples += lstats->tab.counts.delta_dead_tuples - lstats->tab.flushed.delta_dead_tuples;
		tabentry->mod_since_analyze += lstats->tab.counts.changed_tuples - lstats->tab.flushed.changed_tuples;
		 ...
		 */
		tabentry->ins_since_vacuum += lstats->tab.counts.tuples_inserted - lstats->tab.flushed.tuples_inserted;
	...
	dbentry->tuples_returned += lstats->tab.counts.tuples_returned - lstats->tab.flushed.tuples_returned;
	dbentry->tuples_fetched += lstats->tab.counts.tuples_fetched - lstats->tab.flushed.tuples_fetched;
	dbentry->blocks_fetched += lstats->tab.counts.blocks_fetched - lstats->tab.flushed.blocks_fetched;
	dbentry->blocks_hit += lstats->tab.counts.blocks_hit - lstats->tab.flushed.blocks_hit;

	if (flush_txn)
	{
		dbentry->tuples_inserted += lstats->tab.counts.tuples_inserted - lstats->tab.flushed.tuples_inserted;
		dbentry->tuples_updated += lstats->tab.counts.tuples_updated - lstats->tab.flushed.tuples_updated;
		dbentry->tuples_deleted += lstats->tab.counts.tuples_deleted - lstats->tab.flushed.tuples_deleted;

		/*
		 * Record everything as flushed while mid-transaction, where the entry
		 * stays to accumulate more counts.  Clear truncdropped in both counts
		 * and flushed so the next truncate is again seen as new and
		...
		if (!xact_boundary)
		{
			...
			lstats->tab.counts.truncdropped = false;
			lstats->tab.flushed.truncdropped = false;
		}
