...
pgstat_attach_shmem(void)
{
	...
	for (PgStat_Kind kind = PGSTAT_KIND_MIN; kind <= PGSTAT_KIND_MAX; kind++)
	{
		...
		{
			pgStatLocal.kind_dsa[kind] =
				dsa_attach_in_place(pgStatLocal.shmem->raw_kind_dsa_area[kind],
									NULL);
			dsa_pin_mapping(pgStatLocal.kind_dsa[kind]);

			pgStatLocal.kind_hash[kind] =
				dshash_attach(pgStatLocal.kind_dsa[kind], &dsh_params,
...
pgstat_detach_shmem(void)
{
	...
	/* Detach dedicated hash tables and DSAs */
	for (PgStat_Kind kind = PGSTAT_KIND_MIN; kind <= PGSTAT_KIND_MAX; kind++)
	{
		if (pgStatLocal.kind_hash[kind] != NULL)
		{
		...
		{
			dsa_detach(pgStatLocal.kind_dsa[kind]);
			pgStatLocal.kind_dsa[kind] = NULL;
		}
	}
				  ...
				  PgStatShared_HashEntry *shhashent)
{
	...
	chunk = dsa_allocate_extended(pgstat_get_dsa_for_kind(kind),
								  kind_info->shared_size,
								  DSA_ALLOC_ZERO | DSA_ALLOC_NO_OOM);
	if (chunk == InvalidDsaPointer)
		return NULL;

	shheader = dsa_get_address(pgstat_get_dsa_for_kind(kind), chunk);
...
pgstat_reinit_entry(PgStat_Kind kind, PgStatShared_HashEntry *shhashent)
{
	...
	shheader = dsa_get_address(pgstat_get_dsa_for_kind(kind), shhashent->body);
...
pgstat_free_entry(PgStatShared_HashEntry *shent, dshash_seq_status *hstat)
{
	...
	dsa_free(pgstat_get_dsa_for_kind(kind), pdsa);
							  ...
							  Datum match_data, TimestampTz ts)
{
	...
	for (int h = 0; h < pgStatLocal.num_hashes; h++)
	{
		...
		while ((p = dshash_seq_next(&hstat)) != NULL)
		{
			...
			header = dsa_get_address(pgstat_get_dsa_for_kind(p->key.kind), p->body);
