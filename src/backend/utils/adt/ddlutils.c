...
static bool fk_backing_constraint_is_blocked(HeapTuple fkTup, Relation conRel,
											  const TableDdlContext *ctx,
											  TableDdlKind *ref_kind_out);
...
/*
 * fk_backing_constraint_is_blocked
 *		For a self-referential FK constraint, determine whether the
 *		PK/UNIQUE constraint it references would not be emitted under
 *		the active filter.  Returns true and sets *ref_kind_out when
 *		the backing constraint's kind is blocked; returns false when
 *		the FK is safe to emit (backing kind is included, or no
 *		matching backing constraint was found).
 */
static bool
fk_backing_constraint_is_blocked(HeapTuple fkTup, Relation conRel,
								  const TableDdlContext *ctx,
								  TableDdlKind *ref_kind_out)
{
	Form_pg_constraint fkcon = (Form_pg_constraint) GETSTRUCT(fkTup);
	TupleDesc	conDesc = RelationGetDescr(conRel);
	Datum		confkeyDatum;
	bool		isnull;
	ArrayType  *confkeyArr;
	int			nconfkey;
	int16	   *confkeyAttrs;
	SysScanDesc conScan;
	ScanKeyData conKey;
	...
	confkeyDatum = heap_getattr(fkTup, Anum_pg_constraint_confkey,
								conDesc, &isnull);
	if (isnull)
		return false;

	confkeyArr = DatumGetArrayTypeP(confkeyDatum);
	nconfkey = ArrayGetNItems(ARR_NDIM(confkeyArr), ARR_DIMS(confkeyArr));
	confkeyAttrs = (int16 *) ARR_DATA_PTR(confkeyArr);

	ScanKeyInit(&conKey, Anum_pg_constraint_conrelid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(fkcon->confrelid));
	conScan = systable_beginscan(conRel, ConstraintRelidTypidNameIndexId,
								 true, NULL, 1, &conKey);

	while (HeapTupleIsValid(cTup = systable_getnext(conScan)))
	{
		Form_pg_constraint cand = (Form_pg_constraint) GETSTRUCT(cTup);
		Datum		conkeyDatum;
		ArrayType  *conkeyArr;
		int			npk;
		int16	   *conkeyAttrs;
		...
		if (cand->contype != CONSTRAINT_PRIMARY &&
			cand->contype != CONSTRAINT_UNIQUE)
			continue;

		conkeyDatum = heap_getattr(cTup, Anum_pg_constraint_conkey,
								   conDesc, &isnull);
		if (isnull)
			continue;
		conkeyArr = DatumGetArrayTypeP(conkeyDatum);
		npk = ArrayGetNItems(ARR_NDIM(conkeyArr), ARR_DIMS(conkeyArr));

		/*
		 * A valid FK must reference exactly the same number of columns as
		 * the PK/UNIQUE candidate.  The equal-length guard makes the
		 * one-directional subset check below sufficient for exact equality.
		 */
		if (npk != nconfkey)
			continue;
		conkeyAttrs = (int16 *) ARR_DATA_PTR(conkeyArr);

		match = true;
		for (i = 0; i < nconfkey && match; i++)
		{
			bool		found = false;

			for (j = 0; j < npk; j++)
			{
				if (confkeyAttrs[i] == conkeyAttrs[j])
				{
					found = true;
					break;
				}
		...
		kind = (cand->contype == CONSTRAINT_PRIMARY)
			? TABLE_DDL_KIND_PRIMARY_KEY
			: TABLE_DDL_KIND_UNIQUE;
		if (!is_kind_included(ctx, kind))
		{
			/*
			 * This candidate is blocked.  Remember it but keep scanning:
			 * a table can have both a PK and a UNIQUE on the same columns,
			 * and the FK might be satisfiable by the other one if its kind
			 * is included.
			 */
			*ref_kind_out = kind;
			blocked = true;
		...
		{
			/* Found an included backing constraint -- FK is safe to emit. */
			blocked = false;
			break;
		}
	}
		...
		switch (con->contype)
		{
			...
			case CONSTRAINT_FOREIGN:
				if (!is_kind_included(ctx, TABLE_DDL_KIND_FOREIGN_KEY))
					continue;
				/*
				 * Self-referential FK: if the PK/UNIQUE it targets on
				 * the same table is filtered out, the FK can't be
				 * created on replay.  Error now so the emitted DDL
				 * never dangles.
				 */
						...
						ereport(ERROR,
								(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
								 errmsg("self-referential foreign key \"%s\" on table \"%s\" requires kind \"%s\" to be emitted",
										...
										kind_name),
								 errdetail("The key references columns constrained by a \"%s\" constraint, which is not in the active filter.",
										   kind_name),
