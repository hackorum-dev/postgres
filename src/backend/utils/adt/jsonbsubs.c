				  ...
				  ForeachAIteratorOptions *options)
{
	...
	*typmod = -1;
	...
	if (!options || options->coercion_strategy == RETURNS_VALUE_OF_COMMON_TYPE)
	{
	...
	if (options->coercion_strategy == RETURNS_COERCED_VALUE)
	{
		Jsonb	   *jb = JsonbValueToJsonb(jbv);

		result = json_populate_type(PointerGetDatum(jb), JSONBOID,
									options->target_typid, options->target_typmod,
									cache, mcxt,
									isnull, false, NULL);

		*typid = options->target_typid;
		*typmod = options->target_typmod;

		return result;
	}

	if (options->coercion_strategy == RETURNS_ASSIGNMENTABLE_VALUE)
	{
		Oid			target_typid = options->target_typid;
		int32		target_typmod = options->target_typmod;
		...
		elog(ERROR, "unknown ForeachAIterator coercion strategy");
}
	...
	while ((r = JsonbIteratorNext(&iter->it, &jbv, iter->skip_nested)) != WJB_DONE)
	{
		...
		if (r == WJB_ELEM)
		{
			*value = JsonbValueToDatum(&jbv, typid, typmod, isnull,
									   &iter->cache, iter->cache_mcxt, options);
...
create_jsonb_foreach_a_iterator(Datum value, Oid typid, int32 typmod, int slice)
{
