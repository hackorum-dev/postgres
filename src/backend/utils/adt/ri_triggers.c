...
typedef struct FastPathMeta
{
	 ...
	 * their own, created with this struct and destroyed with it in
	 * AtEOXact_RI().
	 *
					  ...
					  RI_ConstraintInfo *riinfo)
{
	 ...
	 * The probe below runs user-defined cast and equality functions, which can
	 * accept invalidation messages; InvalidateConstraintCacheCallBack() then
	 * clears riinfo->fpmeta, so re-reading it partway through the batch would
	 * find NULL.  The object itself stays valid until AtEOXact_RI().
	 */
