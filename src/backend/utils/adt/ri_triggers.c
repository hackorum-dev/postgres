...
ri_FastPathGetEntry(const RI_ConstraintInfo *riinfo, Relation fk_rel)
{
	...
	if (!found)
	{
		 ...
		 * LockRelationOid() processes invalidation messages after acquiring the
		 * lock, so we either see the new index, or an old one that cannot be
		 * marked dead or dropped until this transaction ends.
		 */
		riinfo = ri_LoadConstraintInfo(riinfo->constraint_id);
