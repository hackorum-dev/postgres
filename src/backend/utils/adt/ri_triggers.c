 ...
 *
 * The SPI path had these checks done by ExecInitFunc() when initializing
 * the generated query: the equality operator's function appears in its WHERE
 * clause and the cast function, if any, in the cast applied to the parameter.
 * Call with the user id already switched to the referenced table's owner.
 */
static void
