...
int4um(PG_FUNCTION_ARGS)
{
	...
	if (unlikely(pg_neg_s32_overflow(arg, &result)))
		ereport(ERROR,
				(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
...
int4div(PG_FUNCTION_ARGS)
{
	...
	if (arg2 == -1)
	{
		if (unlikely(pg_neg_s32_overflow(arg1, &result)))
			ereport(ERROR,
					(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
					 errmsg("integer out of range")));
		PG_RETURN_INT32(result);
...
int2um(PG_FUNCTION_ARGS)
{
	...
	if (unlikely(pg_neg_s16_overflow(arg, &result)))
		ereport(ERROR,
				(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
				 errmsg("smallint out of range")));
...
int2div(PG_FUNCTION_ARGS)
{
	...
	if (arg2 == -1)
	{
		if (unlikely(pg_neg_s16_overflow(arg1, &result)))
			ereport(ERROR,
					(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
					 errmsg("smallint out of range")));
		PG_RETURN_INT16(result);
...
int2abs(PG_FUNCTION_ARGS)
{
	...
	result = arg1;
	if (arg1 < 0 && unlikely(pg_neg_s16_overflow(arg1, &result)))
		ereport(ERROR,
				(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
				 errmsg("smallint out of range")));
	PG_RETURN_INT16(result);
...
int4lcm(PG_FUNCTION_ARGS)
{
	...
	if (result < 0 && unlikely(pg_neg_s32_overflow(result, &result)))
		ereport(ERROR,
				(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
				 errmsg("integer out of range")));
