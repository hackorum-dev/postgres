...
static void XmlTableSetNamespace(struct TableFuncScanState *state, const char *name,
					 const char *uri);
static void XmlTableSetDefaultNamespaceName(TableFuncScanState *state,
					 const char *name);
...
const TableFuncRoutine XmlTableRoutine =
{
	...
	XmlTableSetDefaultNamespaceName,
...
/* XPath default namespace support */
#ifdef USE_LIBXML
...
/* XPath parser with possibility one push back token */
typedef struct XPathParserData
{
	const char	   *cur;
...
static void transform_xpath(StringInfo str,
						const char *xpath,
						const char *def_namespace_name);
static const char *next_xpath_token_unbuffered(const char *str,
						XPathTokenInfo *ti);
static void init_xpath_parser(XPathParserData *parser, const char *str);
static void next_xpath_token(XPathParserData *parser, XPathTokenInfo *ti);
static void push_xpath_token(XPathParserData *parser, XPathTokenInfo *ti);
static void write_xpath_token(StringInfo str, XPathTokenInfo *ti);
static void transform_xpath_recurse(StringInfo str,
			   ...
			   int *res_nitems, ArrayBuildState *astate)
{
	...
	PG_TRY();
	{
		...
		if (ns_count > 0)
		{
			...
			for (i = 0; i < ns_count; i++)
			{
				...
				if (*ns_name == '\0')
				{
					...
					default_ns_uri = ns_uri;
				}

				/*
				 * collect ns names in ResTarget format for possible usage
				 * in getUniqNames function.
				 */
				rt = makeNode(ResTarget);
				rt->name = ns_name;
				names = lappend(names, (Node *) rt);

				/* register namespace if all proprieties are available */
				if (*ns_name != '\0' && xmlXPathRegisterNs(xpathctx,
										   (xmlChar *) ns_name,
										   (xmlChar *) ns_uri) != 0)
			...
			/* get default namespace name if any */
			if (default_ns_uri)
			{
				default_ns_name = getUniqName(names);

				if (xmlXPathRegisterNs(xpathctx,
									   (xmlChar *) default_ns_name,
									   (xmlChar *) default_ns_uri) != 0)
					ereport(ERROR,	/* is this an internal error??? */
							(errmsg("could not register XML namespace with name \"%s\" and URI \"%s\"",
									default_ns_name, default_ns_uri)));
			}
		}
		...
		if (default_ns_name)
		{
			...
			transform_xpath(&str, text_to_cstring(xpath_expr_text), default_ns_name);
 ...
 */
static void
XmlTableSetDefaultNamespaceName(TableFuncScanState *state, const char *name)
{
#ifdef USE_LIBXML
	XmlTableBuilderData *xtCxt;
...
XmlTableSetRowFilter(TableFuncScanState *state, const char *path)
{
	...
	if (xtCxt->default_ns_name)
	{
		StringInfoData		str;

		transform_xpath(&str, path, xtCxt->default_ns_name);
		path = str.data;
	}
 ...
 *
 * We introduce new unique namespace name, and we add this name to any
 * unqualified node name or attribute name of original xpath expression.
 * This is outer (libxml2) implementation of the default namespaces.
 *
 * This transformation is implemented by a simple single-purpose XPath
 * preprocessor. It doesn't replace libxml2 XPath parser or libxml2 XPath
 * expression evaluation.
 *
 * Unqualified identifiers of XPath expression entered in "xpath" parameter
 * are qualified by name specified "def_namespace_name" parameter. Result
 * is returned as content of StringInfo "str" argument.
 */
static void
transform_xpath(StringInfo str, const char *xpath, const char *def_namespace_name)
{
	XPathParserData parser;
	...
	initStringInfo(str);
	init_xpath_parser(&parser, xpath);
	transform_xpath_recurse(str,
							&parser,
							false,
							def_namespace_name);
}
...
/* XML tag can use char, dash, dot and digit */
#define IS_NODENAME_CHAR(c)		(IS_NODENAME_FIRSTCHAR(c) || \
								(c) == '-' || \
								(c) == '.' || \
								((c) >= '0' && (c) <= '9'))
...
/*
 * Fill XPathTokenInfo by next token data. Returns pointer to
 * first char after detected token.
 */
const char *
next_xpath_token_unbuffered(const char *str, XPathTokenInfo *ti)
{
	/* skip initial spaces */
	while (*str == ' ')
		str++;
...
/*
 * Initialize XPath parser data.
 *		Token buffer is empty on start
 */
static void
init_xpath_parser(XPathParserData *parser, const char *str)
{
	parser->cur = str;
...
/*
 * Returns one XPath token.
 *		Use buffered token when it is possible
 */
static void
next_xpath_token(XPathParserData *parser, XPathTokenInfo *ti)
{
	if (!parser->buffer_is_empty)
	{
		...
		parser->cur = next_xpath_token_unbuffered(parser->cur, ti);
}

/*
 * Push token to parser's buffer
 */
static void
push_xpath_token(XPathParserData *parser, XPathTokenInfo *ti)
{
	Assert(parser->buffer_is_empty);

	memcpy(&parser->buffer, ti, sizeof(XPathTokenInfo));
	parser->buffer_is_empty = false;

	/* token info doesn't hold any data now */
	ti->ttype = XPATH_TOKEN_NONE;
}

/*
 * Write original token to output string info
 */
static void
write_xpath_token(StringInfo str, XPathTokenInfo *ti)
{
...
/*
 * transform_xpath_recurse
 *
 * This function iterate over tokens of XPath expressions.
 * It writes def_namespace_name before any unqualified name or unqualified
 * attribute.
 *
 * XPath expression can contains predicates - for this transformation we
 * process predicates as recursive iteration of transform_xpath_recurse.
 * The parameter inside_predicate has true in this case. In this case we
 * searching ']' to ending this function.
 *
 * StringInfo str collects transformed (output) XPath expression Param parser
 * holds parser data.
 */
static void
transform_xpath_recurse(StringInfo str,
						XPathParserData *parser,
						bool inside_predicate,
						const char *def_namespace_name)
{
	...
	/* Guard against stack overflow due to overly complex XPath expressions */
	check_stack_depth();

	next_xpath_token(parser, &t1);
			...
			case XPATH_TOKEN_OTHER:
				{
					...
					if (c == '[')
						transform_xpath_recurse(str, parser, true, def_namespace_name);
					else
					{
