						...
						ForeachAIteratorOptions *options)
{
	...
	*typmod = iter->result_typmod;
	...
	if (options && options->coercion_strategy == RETURNS_COERCED_VALUE
		&& (iter->result_typid != options->target_typid ||
			iter->result_typmod != options->target_typmod))
		elog(ERROR, "Unsupported RETURNS_COERCED_VALUE coercion strategy");

	if (options && OidIsValid(options->target_typid))
	{
...
create_array_foreach_a_iterator(Datum value, Oid typid, int32 typmod, int slice)
{
