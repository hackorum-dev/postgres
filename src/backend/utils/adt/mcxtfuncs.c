 ...
 *
 * By default, only superusers are allowed to signal a process to return the
 * memory contexts.  Additional roles can be permitted with GRANT.
 *
 * On receipt of this signal, a backend or an auxiliary process sets the flag
 * in the signal handler, which causes the next CHECK_FOR_INTERRUPTS()
...
pg_get_process_memory_contexts(PG_FUNCTION_ARGS)
{
	...
	/*
	 * BackendPidGetProc() and AuxiliaryPidGetProc() return NULL if the pid
	 * isn't valid; this is however not an error and leave with a WARNING.
	 * Refer to comments in pg_log_backend_memory_contexts for a discussion on
	 * this.
	 */
	if (proc == NULL)
	{
...
/*
 * memstats_dsa_clenaup
 *		Release allocated DSA memory for memory context reporting
 *
 * Before returning from pg_get_process_memory_contexts() this function is
 * executed in order to release DSA memory allocated for storing the memory
 * context statistics.  The caller must ensure that the MemoryStatsDsHash
 * LWLock isn't already held.
 */
static void
...
/*
 * memstats_client_key_reset
 *
 * Convenience function for removing this process from the publishing process'
...
ProcessGetMemoryContextInterrupt(void)
{
	...
	/*
	 * Create a new memory context which is not a child of TopMemoryContext.
	 * This context is used to allocate all memory in this function.  This
	 * helps in keeping the memory allocation in this function to report
	 * memory consumption statistics separate so that the process of
	 * extracting memory contexts doesn't pollute the report.
	 */
	 ...
	 * similar to how memory context logging for the local backend works.
	 */
	ctl.keysize = sizeof(MemoryContext);
	ctl.entrysize = sizeof(MemoryContextId);
	ctl.hcxt = CurrentMemoryContext;
