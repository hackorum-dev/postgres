...
{
	const TableFuncRoutine *routine = tstate->routine;
	TableFunc *tablefunc = ((TableFuncScan *) (tstate->ss.ps.plan))->tablefunc;
	TupleDesc	tupdesc;
	ListCell   *lc1,
			   *lc2;
	bool		isnull;
	int			colno;
	Datum		value;
	int			ordinalitycol = tablefunc->ordinalitycol;
	char	   *default_ns_name = tablefunc->default_ns_name;
	...
	if (default_ns_name)
		routine->SetDefaultNamespaceName(tstate, default_ns_name);
