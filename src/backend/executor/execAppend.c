...
ExecReScanAppendBase(AppendBaseState *node)
{
	...
	if (nasyncplans > 0)
	{
		...
		while ((i = bms_next_member(node->asyncplans, i)) >= 0)
		{
			...
			/*
			 * Any request still pending a callback must already have been
			 * drained by the node type's own rescan, which calls
			 * ExecAppendBaseAsyncProcessPending() before reaching here.  We
			 * rely on that so that we don't leave the async-capable node's
			 * per-connection state (e.g. postgres_fdw's pendingAreq) out of
			 * step with ours.
			 */
...
/*
 * ExecAppendBaseAsyncProcessPending
 *		Process all of the asynchronous requests still pending.
 *
 * When a rescan finds some async request still pending a callback (e.g.
 * because a LIMIT above us stopped pulling tuples before it completed), the
 * request must be drained before we discard the derived state: otherwise the
 * async-capable node's own per-connection state (e.g. postgres_fdw's
 * pendingAreq on a connection it may share between subplans) would be left out
 * of step with ours.  We drain by waiting for and delivering the outstanding
 * requests until none remain pending.  'timeout' and 'wait_event_info' are
 * passed through to ExecAppendBaseAsyncEventWait() and let each node type keep
 * its own waiting behavior (Append polls when not syncdone; MergeAppend always
 * waits).
 */
void
ExecAppendBaseAsyncProcessPending(AppendBaseState *node, int timeout,
								  uint32 wait_event_info)
{
	for (;;)
	{
		...
		ExecAppendBaseAsyncEventWait(node, timeout, wait_event_info);
	}
}
...
ExecAppendBaseAsyncBegin(AppendBaseState *node)
{
	...
	while ((i = bms_next_member(node->valid_asyncplans, i)) >= 0)
	{
		...
		/*
		 * Any request still pending a callback from before a rescan was
		 * already drained during the rescan (see ExecReScanAppend() /
