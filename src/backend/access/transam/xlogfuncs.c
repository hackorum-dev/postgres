 ...
 * insertion does not have to create and zero-fill segments itself.  Unless
 * force is true, the request is limited to max_wal_size.
 *
...
{
	int64		bytes;
	bool		force = !PG_ARGISNULL(1) && PG_GETARG_BOOL(1);
	int64		nsegs;
	int64		nsegsadded;
			...
			if (!PG_ARGISNULL(0))
				ereport(NOTICE,
						(errmsg("WAL preallocation request was reduced to fit "
						 ...
						 errhint("Call pg_wal_preallocate() with \"force\" set to true "
