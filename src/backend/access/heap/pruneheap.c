					  ...
					  MultiXactId *new_relmin_mxid)
{
	...
	/*
	 * Do not split a later cleanup-lock pass into separate freeze and prune
	 * WAL records.  Save freeze plans locally while examining each tuple, and
	 * discard them if an old DEAD tuple requires a cleanup-lock pass. Process
	 * items in reverse order, like prune_freeze_plan(), to improve cache
	 * locality when tuple data was placed at decreasing offsets.
	 */
		 ...
		 offnum = OffsetNumberPrev(offnum))
	{
		...
		*off_loc = offnum;
		...
		htsv = heap_tuple_satisfies_vacuum(&tuple, params->buffer,
		...
		switch (htsv)
		{
			...
			case HEAPTUPLE_DEAD:
				result->dead_tuples++;
				if (heap_tuple_should_freeze(tupleheader, params->cutoffs,
											 &dead_relfrozen_xid,
											 &dead_relmin_mxid))
