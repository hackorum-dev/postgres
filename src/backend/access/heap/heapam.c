...
l2:
	 ...
	 * transform this update into an insert during decoding; without the old
	 * tuple having external data there is nothing to preserve.
	 */
 ...
 * tuple, see pgoutput_row_filter()).  heap_update() requests this when the
 * table is published with a row filter; the extra columns only matter when the
 * key is logged too, because a row filter can only reference replica identity
 * columns, so the transformation requires a key change.
 *
