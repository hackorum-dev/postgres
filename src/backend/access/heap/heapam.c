 ...
 *
 * Not public API.  This exists only to serve HeapUpdateModifiedIdxAttrs (and
 * thus simple_heap_update); nothing else -- core or out-of-tree -- should call
 * it.  It is targeted for eventual removal, once catalog-tuple updates track
 * their own changed columns and simple_heap_update no longer has to
 * re-discover them.  New callers that need "did an indexed attribute change?"
 * on the executor path use ExecCompareSlotAttrs (which intentionally uses a
 * different, storage-agnostic comparator; see its comment and
 * README.HOT-INDEXED section 6).
 */
static bool
 ...
 *
 * This is the non-executor counterpart of ExecUpdateModifiedIdxAttrs /
 * ExecCompareSlotAttrs, and the two are meant to be *functionally* equivalent
 * (both answer "which indexed attributes changed value?").  They intentionally
 * differ in the Datum comparator they use: this heap-side path uses
 * heap_attr_equals (datumIsEqual), the executor path uses datum_image_eq --
 * see ExecCompareSlotAttrs and README.HOT-INDEXED section 6 for why.
 *
 * Not public API.  Its only caller is simple_heap_update() (catalog and other
 * non-executor updates that lack a ResultRelInfo/EState to supply the changed
 * columns), and it is targeted for eventual removal once catalog-tuple updates
 * track their own changes.  Nothing else -- core or out-of-tree -- should
 * depend on it; executor callers use ExecUpdateModifiedIdxAttrs.
 */
static Bitmapset *
