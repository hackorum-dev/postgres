 ...
 *
 * Currently only 2-way joins are supported; warns and returns NULL for
 * n-way (njoinrels > 1).
 */
MCVList *
					   ...
					   VacAttrStats ***result_stats)
{
	...
	/* Open the other table (joinrel_oids[0]) */
	other_rel = table_open(joinrel_oids[0], AccessShareLock);
 ...
 *	  Match a rel's base restrictions against a stat's stxkeys.
 *
 * For each restriction clause on rel that extract_filter_info can parse,
 * check whether the filtered column appears in the stat's stxkeys with a
 * matching keyref value.  Matched filters are appended to the output lists.
 *
 * keyref_match is the keyrefs value that identifies this rel's columns in
 * the stat: 1 for the anchor rel, != 1 for the other rel.
 */
static void
					  ...
					  List **filter_collations)
{
	...
	foreach(lc, rel->baserestrictinfo)
	{
		...
		forboth(lc_attr, stat->keyattrs, lc_ref, stat->keyrefs)
		{
			bool		ref_matches = is_anchor
				? (lfirst_int(lc_ref) == 1)
				: (lfirst_int(lc_ref) != 1);

			if (lfirst_int(lc_attr) == filter_var->varattno && ref_matches)
			{
				key_idx = pos;
				break;
			}
				...
				List **filter_collations)
{
	...
	*filter_collations = NIL;
	 ...
	 * this stat's stxkeys.  Without at least one covered filter, the stat
	 * cannot improve on the standard join selectivity estimate, so bail out.
	 *
	 * For other_rel filters, match stxkeys columns with keyrefs != 1 (joined
	 * relation columns).  For anchor_rel filters, match stxkeys columns with
	 * keyrefs == 1 (anchor relation columns).
	 */
