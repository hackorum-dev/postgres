...
AfterTriggerSetState(ConstraintsSetStmt *stmt)
{
	...
	if (!stmt->deferred)
	{
		 ...
		 * already running, but unlike AfterTriggerEndQuery() it opens no
		 * query level.  Batch callbacks registered by the triggers fired
		 * below would therefore land in the same list as the enclosing
		 * cycle's, which fires that list at its own end.  Swap in an empty
		 * list so this cycle fires and frees only its own callbacks: firing
		 * the enclosing cycle's here would flush its batch early, under a
		 * resource owner that can go away before the batch is used again, and
		 * leave it with nothing to flush when it does end.
		 *
		...
		*afterTriggerBatchCallbackList(cb_depth) = NIL;
		...
		PG_TRY();
		{
			while (afterTriggerMarkEvents(events, NULL, true))
			{
				 ...
				 * level, but we'd better not if inside a subtransaction,
				 * since the subtransaction could later get rolled back.
				 */
			 ...
			 * fired.  SET CONSTRAINTS ... IMMEDIATE means "check these now",
			 * so nothing may be left buffered here: a pending batch would
			 * report success for a constraint that was never actually
			 * checked.
			 */
