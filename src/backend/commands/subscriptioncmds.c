...
AlterSubscription_refresh_table(Subscription *sub, List *relations)
{
	...
	foreach_oid(relid, relids)
	{
		...
		/*
		 * ExecuteTruncateGuts() runs the checks below only for the relations
		 * it adds itself through CASCADE, leaving them to the caller for the
		 * ones passed in.  ExecuteTruncate() calls truncate_check_rel(),
		 * truncate_check_perms() and truncate_check_activity() for those; of
		 * what they do, the relkind and system catalog restrictions cannot be
		 * violated by a relation that is in pg_subscription_rel, the
		 * permission check is done in the first pass above, and a partition
		 * cannot be another backend's temp table.  What is left is the
		 * truncate hook, which an audit or label provider needs to see for a
		 * command that discards a table's contents, and the check for uses of
		 * the relation in the current transaction.
		 */
		InvokeObjectTruncateHook(relid);
		CheckTableNotInUse(userrel, "ALTER SUBSCRIPTION ... REFRESH TABLE");
	}
