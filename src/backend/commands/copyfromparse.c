 ...
 *   IN_ARRAY       — inside [...]; expect object, separators, or ']'.
 *                    Another '{' after an object requires a preceding ',' (JSON).
 ...
 */
static bool
copy_json_refill_if_exhausted(CopyFromState cstate, int *obj_start)
{
	...
	if (json_state->parse_pos > 0 &&
		...
		json_state->parse_state != JSON_PARSE_IN_STRING &&
		json_state->parse_state != JSON_PARSE_IN_STRING_ESC)
	{
	...
	if (json_state->parse_state == JSON_PARSE_IN_OBJECT ||
		json_state->parse_state == JSON_PARSE_IN_STRING ||
		json_state->parse_state == JSON_PARSE_IN_STRING_ESC)
	{
	...
	if (json_state->array_mode &&
		json_state->parse_state == JSON_PARSE_IN_ARRAY)
	{
		if (cstate->cur_lineno == 0)
			cstate->cur_lineno = 1;
		cstate->line_buf_valid = false;
 ...
 * (see CopyReadNextJson, JSON_PARSE_IN_OBJECT); call that index tail_from.
 ...
 */
static void
copy_json_finalize_linebuf_for_row(CopyFromState cstate, StringInfo line_buf)
{
...
CopyReadNextJson(CopyFromState cstate)
{
	...
	for (;;)
	{
		if (!copy_json_refill_if_exhausted(cstate, &obj_start))
		{
			cstate->line_buf_valid = false;
			return true;
		}
		...
		while (json_state->parse_pos < line_buf->len)
		{
			...
			switch (json_state->parse_state)
			{
				...
				case JSON_PARSE_IN_ARRAY:
					if (c == ']')
					{
						json_state->parse_state = JSON_PARSE_ARRAY_END;
						cstate->line_buf_valid = false;
						return true;
					}
					if (c == '{')
						...
						case '}':
							...
							if (json_state->object_depth == 0)
							{
								...
								json_state->parse_state = (json_state->array_mode)
									? JSON_PARSE_IN_ARRAY : JSON_PARSE_BEFORE_OBJECT;
								 ...
								 * consumed in JSON_PARSE_IN_ARRAY; reject
								...
								copy_json_finalize_linebuf_for_row(cstate, line_buf);
								return false;
							}
							break;
						case '[':
				...
				case JSON_PARSE_IN_STRING_ESC:
					json_state->parse_state = JSON_PARSE_IN_STRING;
					break;

				case JSON_PARSE_ARRAY_END:
					cstate->line_buf_valid = false;
					return true;
			}
		}
