					...
					QueryCompletion *qc)
{
	...
	if (RelationIsIMMV(matviewRel) && skipData)
	{
		Relation	tgRel;
		Relation	depRel;
		ScanKeyData key;
	 ...
	 * is created.
	 */
	if (RelationIsIMMV(matviewRel) && !skipData && !oldPopulated)
	{
	...
	if (!skipData)
	{
		...
		if (RelationIsIMMV(matviewRel) && !IsolationUsesXactSnapshot())
			PopActiveSnapshot();
	}
