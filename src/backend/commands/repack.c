...
typedef struct ChangeContext
{
	...
	/*
	 * The relation the changes were decoded from.  Its descriptor is the one
	 * to deform the decoded tuples with, see apply_concurrent_changes().
	 */
	Relation	cc_src_rel;
...
apply_concurrent_changes(BufFile *file, ChangeContext *chgcxt)
{
	...
	/*
	 * Deform the decoded tuples with the descriptor of cc_src_rel, which is
	 * the only one that has the missing values.  Tuples formed with it are
	 * still valid for cc_rel, whose attributes are a copy of cc_src_rel's.
	 */
	spilled_tuple = MakeSingleTupleTableSlot(RelationGetDescr(chgcxt->cc_src_rel), &TTSOpsVirtual);
	ondisk_tuple = MakeSingleTupleTableSlot(RelationGetDescr(rel),
											table_slot_callbacks(rel));
	old_update_tuple = MakeSingleTupleTableSlot(RelationGetDescr(chgcxt->cc_src_rel), &TTSOpsVirtual);
						  ...
						  Oid ident_index_id)
{
	...
	chgcxt->cc_src_rel = src_relation;
