...
start_repack_decoding_worker(Oid relid)
{
	 ...
	 * is gone instead of blocking on the queue. parallel.c binds its error
	 * queues the same way.
	 */
	 ...
	 *
	 * We wait on our latch, not on the condition variable. The worker sets
	 * our latch once it is initialized, and the postmaster sends us SIGUSR1
	 * via bgw_notify_pid if the worker fails to start or exits, which sets
	 * our latch too. A condition variable would not do here: it returns only
	 * on its own signal, not on such a latch wakeup, so a worker that never
	 * starts (e.g. fork failure) would leave us waiting forever while holding
	 * ShareUpdateExclusiveLock on the table.
	 */
	for (;;)
	{
		bool		initialized;
...
stop_repack_decoding_worker(void)
{
	 ...
	 * exit. DestroyParallelContext() detaches first for the same reason.
	 */
