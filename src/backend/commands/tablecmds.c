	...
	while (HeapTupleIsValid(depTup = systable_getnext(depScan)))
	{
		...
		if (relation_expr_has_rowtype_const(rel, typeOid))
		{
			...
			if (origTypeName)
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("cannot alter type \"%s\" because an expression of \"%s\" stores a constant of that type",
								origTypeName, relname),
						 errdetail("The stored constant holds a row image that the altered type would misinterpret.")));
			else
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("cannot alter table \"%s\" because an expression of \"%s\" stores a constant of its row type",
								RelationGetRelationName(origRelation),
								relname),
						 errdetail("The stored constant holds a row image that the altered type would misinterpret.")));
		}
