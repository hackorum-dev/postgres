...
ATPrepAddGenStored(Relation rel, AlterTableCmd *cmd, bool recurse, bool recursing)
{
	...
	if (!recursing && !recurse &&
		...
		ereport(ERROR,
				...
				errdetail("Converting only part of a partitioning/inheritance hierarchy is not supported."),
				errhint("Use this command on the root partition/table without specifying ONLY."));
