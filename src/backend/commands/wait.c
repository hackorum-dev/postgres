			 ...
			 DestReceiver *dest)
{
	 ...
	 * advance far enough to satisfy our wait.  This can create a cycle:
	 * we wait for recovery, while recovery waits for us to release the lock.
	 *
	 * WAIT FOR LSN does not register its dependency on WAL progress as a
	 * lock wait, so the deadlock detector cannot see the complete cycle.
	 * With unlimited recovery-conflict delays and no other timeout or
	 * cancellation, the cycle can persist indefinitely.
	 *
	 * Write and flush waits can also depend on startup.  Without an active
	 * receiver, their replay floor can be their only source of progress,
	 * so holding a lock needed by replay can create the same cycle.
	 *
	 * Streaming can initially provide independent progress, but reception
	 * can stop before the target is reached.  Restarting reception requires
