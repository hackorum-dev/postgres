			 ...
			 DestReceiver *dest)
{
	...
	/*
	 * An unsatisfied standby LSN wait must not retain heavyweight locks.
	 ...
	 * Write and flush waits can also depend on startup during archive recovery
	 * or when WAL reception needs to be restarted.
	 */
	if ((lsnType == WAIT_LSN_TYPE_STANDBY_REPLAY ||
		...
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("cannot wait for a standby LSN while holding heavyweight locks"),
