	...
	foreach(lc, options)
	{
		...
		else if (strcmp(defel->defname, "publish_via_partition_root") == 0)
		{
			if (opts->publish_via_partition_root_given)
				errorConflictingDefElem(defel, pstate);
			opts->publish_via_partition_root_given = true;
		...
		else if (strcmp(defel->defname, "publish_generated_columns") == 0)
		{
			if (opts->publish_generated_columns_given)
				errorConflictingDefElem(defel, pstate);
			opts->publish_generated_columns_given = true;
...
CreatePublication(ParseState *pstate, CreatePublicationStmt *stmt)
{
	...
	if (stmt->for_all_sequences &&
		(opts.publish_given || opts.publish_via_partition_root_given ||
		 opts.publish_generated_columns_given))
		ereport(NOTICE,
						...
						Relation rel, HeapTuple tup)
{
	 ...
	 * this case.
	 */
	if (!pubform->puballtables && opts.publish_via_partition_root_given &&
		!opts.publish_via_partition_root)
	{
		/*
