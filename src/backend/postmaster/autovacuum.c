 ...
 *
 * This looks up and, for TOAST tables, merges reloptions.  Returns NULL if the
 * relation neither sets nor inherits anything.  main_opts returns main table's
 * reloptions, if any.  free_relopts returns whether return value is palloc'd.
 */
static StdRdOptions *
get_effective_relopts(HeapTuple tup, TupleDesc desc, HTAB *toast_map,
					  StdRdOptions **main_opts, bool *free_relopts)
{
	...
	*free_relopts = (relopts != NULL);

	/* if we're a TOAST table, look up our parent's relopts, too */
	if (classForm->relkind == RELKIND_TOASTVALUE)
		hentry = hash_search(toast_map, &classForm->oid, HASH_FIND, &found);
	*main_opts = found ? &hentry->ar_reloptions : NULL;
