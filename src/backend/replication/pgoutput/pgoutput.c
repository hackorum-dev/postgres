...
get_rel_sync_entry(PGOutputData *data, Relation relation)
{
	...
	if (!entry->replicate_valid)
	{
		 ...
		 * finalized reports no ancestors, even though relispartition is
		 * still set. It is handled below like the standalone table it has
		 * effectively become, which is also how it is handled once the detach
		 * completes.
		 */
		if (am_partition)
		{
