...
apply_handle_stream_abort(StringInfo s)
{
	...
	/*
	 * Record the subxid, not the top-level xid, so that the error context
	 * names the subtransaction being aborted.  See
	 * set_remote_transaction_info().
	 */
					   ...
					   XLogRecPtr lsn)
{
	...
	/*
	 * No need to record the finish LSN here: the leader apply worker has
	 * already set it in the remote transaction context when processing the
	 * transaction finish message, and in the parallel apply worker it is
	 * invalid, just like the lsn passed by the caller there.
	 */
...
{
	ApplyRemoteCtx *errarg = &remote_ctx;

	if (remote_ctx.command == 0)
		return;
 ...
 * streaming, where other transactions can be applied to completion in
 * between one transaction's streamed chunks) would otherwise leave stale
 * values from whichever transaction last called this.
 *
 * Callers normally pass the top-level transaction's xid.  A subtransaction
 * abort instead passes the subxid and its abort LSN, because the error
 * context should name the subtransaction that failed.  Nothing else observes
 * those, because no change is applied between a STREAM ABORT and the STREAM
 * START or STREAM COMMIT/PREPARE that follows it, and each of those calls
 * this again with the transaction's own values.  The assertion in
...
{
	remote_ctx.origin_name = MemoryContextStrdup(ApplyContext,
												 originname);
}
