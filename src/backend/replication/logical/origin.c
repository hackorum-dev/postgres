						  ...
						  Relation rel)
{
	...
	/*
	 * Advance the origin's tracking state if required. We cannot distinglish
	 * only by the remote_lsn, since a missing status row and a status row at
	 * 0/0 (which still needs to be tracked) both arrive as InvalidXLogRecPtr.
	 */
	if (need_advance)
	{
