 ...
 * occurs there they abort that subxact and, if a handler matches, catch the
 * error without re-throwing it. The top-level handler is then not reached and
 * the slot stays acquired. Release it when the subxact that acquired it aborts,
