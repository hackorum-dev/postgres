...
static Node *transformFrameOffset(ParseState *pstate, int frameOptions,
					 ...
					 Node *clause);
static void mutate_name(char *buffer, int size);
	...
	if (rtf->namespaces != NIL)
	{
		...
		foreach(ns, rtf->namespaces)
		{
			...
			else
			{
				...
				default_ns_name = getUniqName(rtf->namespaces);
			}

			/* We represent DEFAULT by a null pointer */
			ns_names = lappend(ns_names,
							   r->name ? makeString(r->name) :
										 makeString(default_ns_name));
		}

		tf->ns_uris = ns_uris;
		tf->ns_names = ns_names;
		tf->default_ns_name = default_ns_name;
	}
...
/*
 * This procedure modify string to generete unique string in every iteration.
 * It generate sequence a, b, .. aa, ba, ca, .. za, ab, bb, cb, .. zb, ac, ...
 */
static void
mutate_name(char *buffer, int size)
{
	char   *ptr = buffer;
	...
	/*
	 * When we are here, we should to increment length. Buffer of
	 * size 10, should be good enough for 26^9 combinations. So
	 * ensure enough size via assert only.
	 */
	Assert(ptr - buffer - 1 < size);
 ...
 * generateUniqName
 *
 * Returns string that holds the name that was not used
 * in list names. Names is list of ResTarget nodes.
 *
 * The name is generated by pettern:
 *   a, b, .. aa, ba, ca, .. za, ab, bb, cb, .. zb, ac, ...
 */
char *
getUniqName(List *names)
{
	char		buffer[10];
	char	   *result = NULL;
	int			i;
	...
	/*
	 * When buffer is good enough long, then we should to find
	 * uniq name after max list_length(ns_names) iterations. This
	* alghoritm is O(N^2), but we can expect low N (mostly 1).
	 */
	for (i = 1; i <= list_length(names); i++)
	{
		ListCell   *lc;
		bool	is_uniq_name = true;

		mutate_name(buffer, sizeof(buffer));
		...
		foreach(lc, names)
		{
			...
			/* generate new mutation when it is not uniq */
