...
/*
 * rowcollapse_key_preserves_rowcoverage
 *
 ...
 */
static bool
rowcollapse_key_preserves_rowcoverage(List *rowcoverage_key_positions,
								  ...
								  KeyJoinRowCollapse *rowcollapse)
{
	...
	{
		if (rowcollapse_key_preserves_rowcoverage(rowcoverage_key_positions,
...
project_key_join_query_facts(KeyJoinFactContext *context, Query *query)
{
	 ...
	 * GROUPING SETS, ROLLUP, and CUBE do not prove uniqueness; if SELECT
	 * DISTINCT is also present, derive uniqueness from that final row collapse.
	 */
	if (query->distinctClause != NIL ||
...
{
	return append_dependency_unique(dependencies, classId, objectId);
}
