 ...
 * are not ordinary expression references: revalidation uses them as a proof
 * graph, so record them with DEPENDENCY_KEYJOIN after the normal expression
 * dependency pass has run.
 */
void
 ...
 *
 * Collect only the proof graph edges embedded in stored KeyJoinNodes.  The
