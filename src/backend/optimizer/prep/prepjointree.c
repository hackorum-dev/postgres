...
static void remove_redundant_nullability_quals_recurse(Node *jtnode,
													   Relids antijoins);
static Node *remove_redundant_nullability_quals(Node *quals, Relids antijoins);
 ...
 * at the end of this phase; see remove_redundant_nullability_quals_recurse.
 *
...
reduce_outer_joins(PlannerInfo *root)
{
	...
	if (!bms_is_empty(state2.anti_reduced))
		remove_redundant_nullability_quals_recurse((Node *) root->parse->jointree,
												   state2.anti_reduced);
}

/*
 * remove_redundant_nullability_quals_recurse
 ...
 * necessarily true.  Removing such quals is not just an optimization: they
 * would generate bogus selectivity estimates, and they would retain references
 * to rels that join removal might later delete.
 */
static void
remove_redundant_nullability_quals_recurse(Node *jtnode, Relids antijoins)
{
	if (jtnode == NULL)
		return;
	if (IsA(jtnode, RangeTblRef))
	...
	else if (IsA(jtnode, FromExpr))
	{
		...
		foreach(l, f->fromlist)
			remove_redundant_nullability_quals_recurse(lfirst(l), antijoins);
		f->quals = remove_redundant_nullability_quals(f->quals, antijoins);
	}
	else if (IsA(jtnode, JoinExpr))
	{
		JoinExpr   *j = (JoinExpr *) jtnode;

		remove_redundant_nullability_quals_recurse(j->larg, antijoins);
		remove_redundant_nullability_quals_recurse(j->rarg, antijoins);
		j->quals = remove_redundant_nullability_quals(j->quals, antijoins);
	}
	else
 ...
 *		Remove redundant IS NULL quals from one implicit-AND qual list.
 */
static Node *
remove_redundant_nullability_quals(Node *quals, Relids antijoins)
{
	if (quals == NULL)
		return NULL;
	if (IsA(quals, List))
	...
	{
		Var		   *var = find_forced_null_var(quals);
