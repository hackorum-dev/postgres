 ...
 *
 * Returns true if every CTE in cteList is either unreferenced (SELECT)
 * or passes the inlineability checks of is_cte_inlineable().  This is used
 * by is_simple_subquery to determine whether a subquery with CTEs can still
 * be pulled up.
 */
bool
SS_all_ctes_inlineable(Query *subquery)
{
	...
	foreach(lc, subquery->cteList)
	{
		...
		/*
		 * Handle the case of unreferenced SELECT CTEs,
		 * which are neither inlined nor materialized.
		 */
		if (cte->cterefcount == 0 && cmdType == CMD_SELECT)
			return false;
