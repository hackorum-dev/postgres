...
max_parallel_hazard(Query *parse)
{
	...
	/*
	 * For a table-modification statement that can be executed in parallel
	 * mode, the worst hazard found among the objects attached to the target
	 * relation counts too; the hazard level is cached in the relation's
	 * relcache entry.
	 */
	if (!max_hazard_found &&
		IsModifySupportedInParallelMode(parse->commandType))
	{
		...
		/*
		 * The target table is already locked by the caller (this is done in
		 * the parse/analyze phase), and remains locked until end of
		 * transaction.
		 */
		target_rel = table_open(rte->relid, NoLock);

		(void) max_parallel_hazard_test(RelationGetParallelDmlSafety(target_rel),
										&context);
 ...
 * specified table-modification statement.  Currently, we support only
 * INSERT.
 *
 ...
 * parallelism isn't supported, to avoid having to do further
 * parallel-safety checks for these.  The parallel safety of the target
 * relation is checked separately in max_parallel_hazard.)
 */
bool
is_parallel_allowed_for_modify(Query *parse)
{
	 ...
	 * example, the leader might generate a combo CID and it would need to
	 * be propagated to the workers.
	 */
								  ...
								  max_parallel_hazard_context *context)
{
	...
	for (attnum = 0; attnum < tupdesc->natts; attnum++)
	{
		...
		/*
		 * Column default expressions are only applicable to INSERT and
		 * UPDATE.
		 */
		if (att->atthasdef)
		{
			...
			if (parallel_dml_hazard_walker(defaultexpr, context))
				return true;
		}
	}
