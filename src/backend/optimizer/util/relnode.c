...
static RestrictInfo *find_clause_by_serial(List *clauses, int rinfo_serial);
...
#ifdef USE_ASSERT_CHECKING
/*
 * Search a list of restriction clauses for one with the given rinfo_serial.
 * Returns NULL if none.
 */
static RestrictInfo *
find_clause_by_serial(List *clauses, int rinfo_serial)
{
	foreach_node(RestrictInfo, rinfo, clauses)
	{
		if (rinfo->rinfo_serial == rinfo_serial)
			return rinfo;
	}
	return NULL;
}
						  ...
						  Relids required_outer)
{
	...
	foreach(lc, baserel->joininfo)
	{
		 ...
		 * but only when they are textually identical, which happens when a
		 ...
		 * first survivor, asserting that later ones are indeed identical;
		...
		if (rinfo->has_clone || rinfo->is_clone)
		{
			...
			{
				Assert(equal(find_clause_by_serial(pclauses,
												   rinfo->rinfo_serial)->clause,
							 rinfo->clause));
				continue;
			}
		}
	...
	/* We should not have accepted more than one clone of any clause */
