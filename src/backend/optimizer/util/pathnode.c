 ...
 *	  Bloom filters against every tuple it produces, and record them as the
 *	  path's expected_filters.
 *
 * This is the generic, not-smart costing fallback: it assumes each filter
 * costs one hash-probe-equivalent (cpu_operator_cost) per input tuple, on
 * top of shrinking the row estimate by the filters' combined selectivity.
 * Callers that can do something smarter (e.g. a CustomScan skipping whole
 * row groups via a zone map) should cost themselves instead of using this.
 */
void
apply_expected_filters(Path *path, List *filters)
{
	...
	path->total_cost += list_length(filters) * cpu_operator_cost * path->rows;
	path->rows = clamp_row_est(path->rows * expected_filters_selectivity(filters));
