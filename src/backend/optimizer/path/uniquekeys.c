							...
							List *restrictlist)
{
	...
	/* Strengthen keys whose columns the inner join's clauses force non-NULL */
	if (jointype == JOIN_INNER)
 ...
 *		Mark a joinrel's non-NULL-aware keys NULL-aware where the inner join's
 ...
 * such a key holds such a Var, no output row has a NULL key column, so the key
 * is really NULL-aware.  Outer-join clauses are not among these, so a key that
 * lost NULL-awareness to null-extension is left alone.
 */
static void
