	...
	foreach(lc, index->indextlist)
	{
		...
		if (slope_match)
		{
				 ...
				 query_pk_cell = lnext(root->query_pathkeys, query_pk_cell))
			{
				...
				{
					/*
					 * ORDER BY x, f(x), g(x) can be satisfied by an index on
					 * x alone for non-volatile functions when btree-equal
					 * values of x are interchangeable (equalimage).  Within
					 * each such group, f(x) is then constant for any
					 * deterministic f, so no monotonicity check is needed.
					 */
					...
					/*
					 * Monotonic claims assume default btree order on both
					 * sides: the index column's input type and the pathkey's
					 * expression type (after domain -> base).  Refuse when
					 * either uses a non-default opfamily - e.g. a custom
					 * domain opclass on ORDER BY x::domain USING <op>, or
					 * int4::oid where the eclass kept only the int4 Var but
					 * the pathkey is oid_ops.
					 */
					if (qpk->pk_slope != MONOTONICFUNC_NONE)
					{
						...
						/*
						 * Check if the query pathkey uses the default
						 * opfamily
						 */
						em = linitial(qpk->pk_eclass->ec_members);
						qpk_default_opclass =
				...
				/*
				 * Didn't match any supported pattern, we can't proceed
				 */
				query_pk_cell = NULL;
				break;
			}
