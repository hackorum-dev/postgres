...
/*
 * Properties of one old leaf partition index that are not preserved by the
 * cloned rebuild definition, captured before ALTER COLUMN TYPE or SET
...
typedef struct PartitionIndexProps
{
	...
	List	   *reloptions;		/* index's own reloptions (untransformed
								 * DefElem list), or NIL */
	char	   *tableSpace;		/* index's own tablespace, or NULL for default */
} PartitionIndexProps;
