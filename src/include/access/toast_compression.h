 ...
 * Its value is one of the ToastCompressionGucValue enum values defined below,
 * which are independent from both the catalog char constants stored in
 * pg_attribute.attcompression and the on-disk compression ID values
 * (TOAST_COMPRESS_xxx from varatt.h).
 */
...
/*
 * Method Registry translation functions.  "cmid" is the on-disk varatt
 * value.
 */
