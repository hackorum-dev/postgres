 ...
 */
#define SK_ISNULL			(1 << 0)	/* sk_argument is NULL */
#define SK_UNARY			(1 << 1)	/* unary operator (not supported!) */
#define SK_ROW_HEADER		(1 << 2)	/* row comparison header (see above) */
#define SK_ROW_MEMBER		(1 << 3)	/* row comparison member (see above) */
#define SK_ROW_END			(1 << 4)	/* last row comparison member */
#define SK_SEARCHARRAY		(1 << 5)	/* scankey represents ScalarArrayOp */
#define SK_SEARCHNULL		(1 << 6)	/* scankey represents "col IS NULL" */
#define SK_SEARCHNOTNULL	(1 << 7)	/* scankey represents "col IS NOT NULL" */
#define SK_ORDER_BY			(1 << 8)	/* scankey is for ORDER BY op */
