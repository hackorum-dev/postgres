...
hstore_out(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		buflen = add_size(buflen, add_size(6, mul_size(2, HSTORE_KEYLEN(entries, i))));
		/* include "" only if nonnull */
		buflen = add_size(buflen, HSTORE_VALISNULL(entries, i) ? 4 :
						  add_size(2, mul_size(2, HSTORE_VALLEN(entries, i))));
	}
