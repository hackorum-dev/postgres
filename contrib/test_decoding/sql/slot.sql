...
-- Test 1: error caught in an EXCEPTION block, which uses a subxact
-- internally. The slot must be released so the next slot operation works.
...
-- Test 2: the handler does not match the error, so it is re-thrown and reaches
-- the top-level handler, which releases the slot. Show only the SQLSTATE for
-- stable output.
...
-- Test 5: an error caught in a subxact that held no slot does not drop
-- the session's temporary slots. This differs from a top-level error (see the
-- XXX in AtEOSubXact_ReplicationSlot()).
