...
#define LXB_URL_DECODE_ERROR 0x1FFFFF
...
static const size_t
lxb_url_scheme_res_length = sizeof(lxb_url_scheme_res) / sizeof(lxb_url_scheme_data_t);
...
static lxb_status_t
lxb_url_callback_length(const lxb_char_t *data, size_t len, void *ctx);

static lxb_codepoint_t
lxb_url_decode_valid_utf_8_single(const lxb_char_t **data,
                                  const lxb_char_t *end);
static lxb_codepoint_t
lxb_url_decode_valid_utf_8_single_reverse(const lxb_char_t **end,
...
again:
    ...
    case LXB_URL_STATE_OPAQUE_PATH_STATE:
        ...
        while (true) {
            ...
            tmp = p;
            cp = lxb_url_decode_valid_utf_8_single(&p, end);
                         ...
                         const lxb_char_t **data, size_t *length)
{
    ...
    while (p < end) {
        ...
        if (c > 0x1F && c < 0x80 && c != 0x20) {
            ...
            tmp = p;
            cp = lxb_url_decode_valid_utf_8_single(&p, end);
            if (cp > 0x1F && cp != LXB_URL_DECODE_ERROR) {
                p = tmp;
                break;
            }
    ...
    while (end > p) {
        tmp = end;
        cp = lxb_url_decode_valid_utf_8_single_reverse(&end, p);

        if (cp > 0x1F && cp != 0x20 && cp != LXB_URL_DECODE_ERROR) {
            end = tmp;
            break;
        }
    }
                   ...
                   lexbor_mraw_t *mraw, lxb_url_host_opt_t opt)
{
    ...
    if (opt & LXB_URL_HOST_OPT_IDNA) {
        ...
        idna = uidna_openUTS46(UIDNA_CHECK_BIDI|UIDNA_CHECK_CONTEXTJ, &errcode);
        if (U_FAILURE(errcode)) {
            return LXB_STATUS_ERROR;
        }

        size = 4096;
        dest = lexbor_malloc(size);
        if (dest == NULL) {
            goto failed_idna;
        }
    ...
    again:
        ...
        pInfo = (UIDNAInfo) UIDNA_INFO_INITIALIZER;
        errcode = U_ZERO_ERROR;

        length = uidna_nameToASCII_UTF8(idna, (const char *) domain->data,
                                        domain->length, dest, size, &pInfo,
                                        &errcode);
        if (U_FAILURE(errcode)) {
            if (errcode == U_BUFFER_OVERFLOW_ERROR) {
                size *= 4;

                tmp = lexbor_realloc(dest, size);
                if (tmp == NULL) {
                    goto failed_idna;
                }
        ...
        lexbor_str_clean(domain);

        p = lexbor_str_append(domain, mraw, (const lxb_char_t *) dest, length);
        if (p == NULL) {
            goto failed_idna;
        }
...
size_t
lxb_url_length(const lxb_url_t *url, bool exclude_fragment)
{
    size_t length;
    const lexbor_str_t *str;
    ...
    length = lxb_url_scheme_length(url);
    length += colon_str.length;

    /* Host. */

    if (url->host.type != LXB_URL_HOST_TYPE__UNDEF) {
        length += dsol_str.length;

        if (lxb_url_includes_credentials(url)) {
            length += url->username.length;
            ...
            length += at_str.length;
        }

        length += lxb_url_host_length(&url->host);

        if (url->has_port) {
            length += colon_str.length;
            length += lxb_url_port_length(url);
        }
    }
        ...
        if (str->data[0] == '/' && str->data[1] == '/') {
            length += dt_str.length;
        }
    }

    length += lxb_url_path_length(&url->path);

    if (url->query.data != NULL) {
        length += qm_str.length;
        length += lxb_url_query_length(url);
    }

    if (!exclude_fragment && url->fragment.data != NULL) {
        length += hs_str.length;
        length += lxb_url_fragment_length(url);
    }
...
size_t
lxb_url_scheme_length(const lxb_url_t *url)
{
    return url->scheme.name.length;
}

size_t
lxb_url_username_length(const lxb_url_t *url)
{
    return url->username.length;
}

size_t
lxb_url_password_length(const lxb_url_t *url)
{
    return url->password.length;
}

size_t
lxb_url_host_length(const lxb_url_host_t *host)
{
    switch (host->type) {
        case LXB_URL_HOST_TYPE_DOMAIN:
        case LXB_URL_HOST_TYPE_OPAQUE:
            return host->u.domain.length;

        case LXB_URL_HOST_TYPE_IPV4:
            return sizeof("000.000.000.000") - 1;

        case LXB_URL_HOST_TYPE_IPV6:
            return sizeof("[FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF]") - 1;
...
static lxb_status_t
lxb_url_idna_to_unicode(const lexbor_str_t *str,
...
again:
    ...
    length = uidna_nameToUnicodeUTF8(idna, (const char *) str->data, str->length,
                                     dest, size, &pInfo, &errcode);
    if (U_FAILURE(errcode)) {
        if (errcode == U_BUFFER_OVERFLOW_ERROR) {
            size *= 4;
    ...
    status = cb((const lxb_char_t *) dest, length, ctx);
...
size_t
lxb_url_host_unicode_length(const lxb_url_host_t *host)
{
    size_t length;
    lxb_status_t status;
    ...
    if (host->type != LXB_URL_HOST_TYPE_DOMAIN
        && host->type != LXB_URL_HOST_TYPE_OPAQUE)
    {
        return lxb_url_host_length(host);
    }

    length = 0;
    status = lxb_url_idna_to_unicode(&host->u.domain,
                                     lxb_url_callback_length, &length);

    return (status == LXB_STATUS_OK) ? length : 0;
}
...
size_t
lxb_url_port_length(const lxb_url_t *url)
{
    return (url->has_port) ? floor(log10(url->port)) + 1 : 0;
}

size_t
lxb_url_path_length(const lxb_url_path_t *path)
{
    return path->str.length;
}

size_t
lxb_url_query_length(const lxb_url_t *url)
{
    return url->query.length;
}

size_t
lxb_url_fragment_length(const lxb_url_t *url)
{
    return url->fragment.length;
}
...
lxb_status_t
lxb_url_serialize_host_unicode(const lxb_url_host_t *host,
                               lexbor_serialize_cb_f cb, void *ctx)
{
    if (host->type != LXB_URL_HOST_TYPE_DOMAIN
        && host->type != LXB_URL_HOST_TYPE_OPAQUE)
    ...
    return lxb_url_idna_to_unicode(&host->u.domain, cb, ctx);
}
...
/*
 * Code from lexbor/encoding/decode.c
 */
static lxb_codepoint_t
lxb_url_decode_valid_utf_8_single(const lxb_char_t **data, const lxb_char_t *end)
{
    lxb_codepoint_t cp;
