				 ...
				 bool qualify_col)
{
	...
	if (rte->rtekind == RTE_FUNCTION)
	{
		if (varattno == 0)
		{
			 ...
			 *
			 * The current push-down policy only absorbs INNER joins with a
			 * function RTE, so the null-supplied case cannot arise from a
			 * remote outer join we emit ourselves.  We still emit the wrapper
			 * unconditionally: it is a no-op for INNER-join rows, and it keeps
			 * the deparsed SQL correct if a future patch relaxes the INNER
			 * restriction (or a higher-level outer join propagates a NULL row
			 * through the foreign scan's tuple).
			 */
			if (qualify_col)
			{
