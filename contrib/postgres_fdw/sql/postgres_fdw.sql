...
-- Test the ExecMergeAppendAsyncProcessPending code path: same shared-connection
-- pruned-out hazard as the Append case earlier, but with ORDER BY forcing a
-- MergeAppend.  async_p2 and async_p3 share a connection; per outer row exactly
-- one of them is pruned while async_p1 (a different connection) always matches,
-- and the inner LIMIT leaves the other shared-connection request outstanding
-- across the rescan.
EXPLAIN (VERBOSE, COSTS OFF)
SELECT o.x, s.b FROM (VALUES (2505), (3505)) o(x), LATERAL (SELECT b FROM async_pt WHERE a = 1505 OR a = o.x ORDER BY b LIMIT 1) s ORDER BY o.x;
SELECT o.x, s.b FROM (VALUES (2505), (3505)) o(x), LATERAL (SELECT b FROM async_pt WHERE a = 1505 OR a = o.x ORDER BY b LIMIT 1) s ORDER BY o.x;
