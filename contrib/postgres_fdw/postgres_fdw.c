					   ...
					   Plan *outer_plan)
{
	...
	else
	{
		...
		/*
		 * For a join rel, the conditions to apply are obtained from the
		 * fdw_private structure (see foreign_join_ok()).  For an upper rel
		 * there are none there and scan_clauses is empty.
		 */
		remote_exprs = extract_actual_clauses(fpinfo->remote_conds, false);
		local_exprs = extract_actual_clauses(fpinfo->local_conds, false);

		/*
		 * A parameterized foreign join additionally receives, via
		 * scan_clauses, the join clauses that were moved into this path
		 * because of the parameterization (see
		 * get_joinrel_parampathinfo_pushdown()).  Enforce the shippable ones
		 * remotely as parameters, and any others locally.  (Upper rels are
		 * never parameterized, so this loop is a no-op for them.)
		 */
		foreach(lc, scan_clauses)
		{
			RestrictInfo *rinfo = lfirst_node(RestrictInfo, lc);
			...
			if (is_foreign_expr(root, foreignrel, rinfo->clause))
				remote_exprs = lappend(remote_exprs, rinfo->clause);
			else
				local_exprs = lappend(local_exprs, rinfo->clause);
		}
...
/*
 * get_foreign_join_param_conds
 *		Identify the join clauses for the given join relation that can be
 *		pushed to the remote server for a path parameterized by
 *		required_outer, and return them as a list of RestrictInfos.
 *
 * These are the clauses that are movable into this join given the
 * parameterization and are safe to send to the remote server.  We consider
 * both "generic" join clauses from the joininfo list and clauses derived from
 * EquivalenceClasses (the latter cover, for example, the equality conditions
 * introduced by a LATERAL reference).
 */
static List *
get_foreign_join_param_conds(PlannerInfo *root, RelOptInfo *joinrel,
							 Relids required_outer)
{
	List	   *result = NIL;
	...
	/* Generic join clauses movable into this join. */
	...
	/* Clauses generated by EquivalenceClasses for this parameterization. */
	ec_clauses = generate_join_implied_equalities(root, join_and_req,
												  required_outer, joinrel,
												  NULL);
	foreach(lc, ec_clauses)
	{
		RestrictInfo *rinfo = lfirst_node(RestrictInfo, lc);
							...
							JoinPathExtraData *extra)
{
	...
	/*
	 * A join relation with lateral references requires parameterized paths.
	 * We can build those for plain SELECTs, but not yet for the cases that
	 * need an EvalPlanQual recheck (UPDATE/DELETE or a locking clause),
	 * because GetExistingLocalJoinPath() only finds unparameterized local
	 * join paths.  So for those cases we still decline to push the join down.
	 */
	 ...
	 *
	 * If the join relation has lateral references, the path is parameterized
	 * by the lateral rels.  Push the join clauses that reference those outer
	 * rels down to the remote server as well, so they are enforced remotely
	 * (as parameters) rather than locally.
	 */
	joinrestrictinfo = extra->restrictlist;
	if (!bms_is_empty(joinrel->lateral_relids))
		joinrestrictinfo =
			list_concat(list_copy(extra->restrictlist),
						get_foreign_join_param_conds(root, joinrel,
