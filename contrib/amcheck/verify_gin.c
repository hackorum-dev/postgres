...
gin_check_posting_tree_parent_keys_consistency(Relation rel, BlockNumber posting_tree_root)
{
	...
	elog(INFO, "processing posting tree at blk %u", posting_tree_root);
	...
	while (stack)
	{
		...
		if (GinPageIsLeaf(page))
		{
			...
			{
				elog(INFO, "blk %u: parent %u highkey (%u, %u), %s",
					 stack->blkno,
					 stack->parentblk,
		...
		else
		{
			...
			if (stack->parentblk != InvalidBlockNumber)
				...
				elog(INFO, "blk %u: root internal posting tree page with %u items", stack->blkno, maxoff);
